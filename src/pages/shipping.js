...
export default async function renderShipping(container) {
  ...
  container.innerHTML = `
    <div x-data="shippingPage">
      ...
      <div class="card mw-lg shipping-form-card" x-show="showForm" x-transition:enter="transition-fade" x-transition:enter-start="op-0" x-transition:enter-end="op-100" x-cloak>
        ...
        <div class="card-body">
          <form @submit.prevent="submitForm" novalidate>
            ...
            <div class="form-group shipping-location-preview" x-show="form.city.trim()" x-transition x-cloak>
              <label class="form-label text-primary"><i class="fas fa-map-marked-alt" aria-hidden="true"></i> ${t('shipping.locationPreview')}</label>
              <div class="shipping-location-frame">
                <iframe 
                  width="100%" 
                  height="100%" 
                  frameborder="0" 
                  ...
                  class="shipping-location-iframe">
                </iframe>
              </div>
            </div>

            <div class="d-flex gap-2 shipping-form-actions">
        ...
        <template x-if="!loading && addresses.length > 0">
          <div class="d-flex flex-column gap-3">
            <template x-for="a in addresses" :key="a.id">
              <div class="card card-sm shipping-address-card">
                <div class="shipping-address-copy">
                  ...
                  <span class="shipping-address-phone" x-text="a.phone"></span>
                </div>
