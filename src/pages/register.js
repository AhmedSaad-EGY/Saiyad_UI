...
Alpine.data('registerForm', () => ({
  ...
  async submit() {
    ...
    const fields = [
      ...
      { element: document.getElementById('regPassword'), required: true, minLength: 8, messages: { minLength: t('auth.passwordMinLength') } },
      { element: document.getElementById('regConfirmPw'), required: true, matches: { element: document.getElementById('regPassword') }, messages: { matches: t('auth.passwordsDoNotMatch') } },
    ];
    if (this.needsLicense) {
      fields.push({ element: document.getElementById('regLicense'), required: true, messages: { required: t('auth.licenseRequired') } });
    }
    ...
    if (!valid) { this.loading = false; return; }
    ...
    try {
      ...
      document.getElementById('registerAlert').innerHTML = `<div class="alert alert-error">${escapeHtml(err.message)}</div>`;
    } finally {
      this.loading = false;
    }
  },
