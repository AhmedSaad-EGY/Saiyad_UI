export const translations = {
  en: {
    // Navbar
    "nav.home": "Home",
    "home.title": "Home",
    "home.metaDesc": "Egypt\u2019s premier fishing marketplace.",
    "nav.products": "Products",
    "nav.auctions": "Auctions",
    "nav.cart": "Cart",
    "nav.login": "Login",
    "nav.register": "Register",
    "nav.dashboard": "Dashboard",
    "nav.myOrders": "My Orders",
    "nav.wallet": "Wallet",
    "nav.myProducts": "My Products",
    "nav.wishlist": "Wishlist",
    "nav.logout": "Logout",
    "nav.searchPlaceholder": "Search fish, gear, auctions\u2026",

    // Home
    "tour.welcome":
      "Browse premium fishing gear from top brands. Find rods, reels, bait, and more.",
    "tour.products":
      "Explore our catalog with advanced search and filters to find exactly what you need.",
    "tour.auctions":
      "Bid in real-time on exclusive fishing equipment auctions. Win great deals!",
    "home.welcome": "Welcome to Sayiad",
    "home.subtitle":
      "The ultimate marketplace for fishing gear and equipment. Buy, sell, and bid on premium fishing products.",
    "home.browseProducts": "Browse Products",
    "home.viewAuctions": "View Auctions",
    "home.qualityGear": "Quality Gear",
    "home.qualityGearDesc": "Premium fishing equipment from trusted sellers",
    "home.liveAuctions": "Live Auctions",
    "home.liveAuctionsDesc": "Bid in real-time on exclusive fishing items",
    "home.fastShipping": "Fast Shipping",
    "home.fastShippingDesc": "Reliable delivery right to your doorstep",
    "home.securePayments": "Secure Payments",
    "home.securePaymentsDesc": "Safe and protected transaction processing",
    "home.latestProducts": "Latest Products",
    "home.activeAuctions": "Active Auctions",
    "home.viewAll": "View All",
    "home.noProducts": "No products yet",
    "home.noAuctions": "No active auctions",
    "home.loadError": "Could not load products.",

    // Auth
    "auth.login": "Login",
    "login.title": "Login",
    "auth.logoutTitle": "Log Out",
    "auth.logoutConfirm": "Are you sure you want to log out?",
    "auth.email": "Email",
    "auth.password": "Password",
    "auth.signIn": "Sign In",
    "auth.signingIn": "Signing in...",
    "auth.noAccount": "Don't have an account?",
    "auth.register": "Register",
    "register.title": "Create Account",
    "auth.fullName": "Full Name",
    "auth.phone": "Phone",
    "auth.birthdate": "Birthdate",
    "auth.role": "Role",
    "auth.customer": "Customer",
    "auth.fisherman": "Fisherman",
    "auth.baitSeller": "Bait Seller",
    "auth.licenseNumber": "Fishing License Number",
    "auth.licenseRequired":
      "Fishing License Number is required for Fisherman registration.",
    "auth.auctioneer": "Auctioneer",
    "auth.createAccount": "Create Account",
    "auth.creatingAccount": "Creating account...",
    "auth.hasAccount": "Already have an account?",
    "register.auctioneerNotice":
      "Auctioneer accounts require Admin approval before activation. You will be able to use basic Customer features immediately and will be notified when approved.",
    "register.pendingUpgrade":
      "Your Auctioneer role request has been submitted. You can log in now and use Customer features while you wait for Admin approval.",
    "register.goToLogin": "Go to Login",
    "auth.registerSuccess":
      "Account created! Check your email to verify before logging in.",
    "auth.loginSuccess": "Login successful!",
    "auth.loggedOut": "Logged out successfully.",
    "auth.showPassword": "Show password",
    "auth.hidePassword": "Hide password",
    "auth.fullNamePlaceholder": "John Doe",
    "auth.emailPlaceholder": "your@email.com",
    "auth.phonePlaceholder": "+1234567890",
    "auth.licensePlaceholder": "FL-123456",
    "auth.passwordRequirements": "Password requirements",
    "auth.confirmPassword": "Confirm Password",
    "auth.passwordsDoNotMatch": "Passwords do not match.",
    "auth.passwordMinLength": "Password must be at least 8 characters.",
    "auth.passwordRequiresUppercase":
      "Must contain at least one uppercase letter.",
    "auth.passwordRequiresLowercase":
      "Must contain at least one lowercase letter.",
    "auth.passwordRequiresDigit": "Must contain at least one digit.",
    "auth.invalidEmail": "Please enter a valid email address.",
    "auth.fieldRequired": "This field is required.",
    "auth.minAgeRequired": "You must be at least {minAge} years old.",
    "auth.invalidSpecialChar":
      "Must contain at least one special character (!@#$%^&*).",
    "auth.ageLabel": "Age: {age} years",
    "auth.invalidAlphanumeric":
      "This field must only contain letters and numbers.",
    "auth.emailAlreadyRegistered": "This email is already registered.",
    "auth.passwordStrength.weak": "Weak",
    "auth.passwordStrength.fair": "Fair",
    "auth.passwordStrength.good": "Good",
    "auth.passwordStrength.strong": "Strong",
    "auth.iAccept": "I accept the",
    "auth.termsAndConditions": "Terms and Conditions",
    "auth.and": "and",
    "auth.privacyPolicy": "Privacy Policy",
    "auth.termsRequired": "Please accept the terms and conditions.",
    "auth.forgotPassword": "Forgot Password?",
    "auth.tooManyAttempts": "Too many failed attempts. Wait {secs} seconds.",
    "auth.sendResetLink": "Send Reset Link",
    "auth.sendingResetLink": "Sending reset link...",
    "auth.resetLinkSent": "Password reset link has been sent to your email.",
    "auth.verificationCode": "Verification Code",
    "auth.verifyCode": "Verify Code",
    "auth.didntGetCode": "Didn't get the code?",
    "auth.resendCode": "Resend Code",
    "auth.resendLink": "Resend Link",
    "auth.verifying": "Verifying...",
    "auth.verificationTimeout":
      "Verification timed out. Please check your email and try logging in.",
    "auth.veryWeak": "Very Weak",
    "auth.weak": "Weak",
    "auth.fair": "Fair",
    "auth.strong": "Strong",
    "auth.veryStrong": "Very Strong",
    "auth.reqLength": "8+ characters",
    "auth.reqUpper": "Uppercase letter",
    "auth.reqLower": "Lowercase letter",
    "auth.reqNum": "Number",
    "auth.reqSpecial": "Special character",
    "auth.invalidToken": "Invalid or expired token.",
    "auth.resetPassword": "Reset Password",
    "auth.newPassword": "New Password",
    "auth.confirmNewPassword": "Confirm New Password",
    "auth.updatingPassword": "Updating password...",
    "auth.passwordResetSuccess":
      "Password updated successfully! Redirecting to login...",
    "auth.emailNotVerified": "Email not verified.",
    "auth.checkInbox":
      "Please check your inbox and click the verification link.",
    "auth.checkSpam": "Didn't get it? Check your spam folder.",
    "auth.resendVerification": "Resend Verification",
    "auth.verificationSent": "Verification email sent!",
    "auth.sending": "Sending...",
    "auth.sent": "Sent!",
    "auth.emailNotFound": "Email Not Found",
    "auth.emailNotFoundRegister":
      "Email not found. Would you like to register?",

    // Products
    "products.gridView": "Grid View",
    "products.listView": "List View",
    "products.title": "Products",
    "products.metaTitle": "Fish & Seafood Products",
    "products.metaDesc": "Browse fresh fish and seafood on Sayiad marketplace.",
    "products.search": "Search products...",
    "products.allCategories": "All Categories",
    "products.allConditions": "All Conditions",
    "products.minPrice": "Min Price",
    "products.maxPrice": "Max Price",
    "products.inStockOnly": "In stock only",
    "products.filters": "Filters",
    "products.sort": "Default",
    "products.priceLowHigh": "Price: Low to High",
    "products.priceHighLow": "Price: High to Low",
    "products.newest": "Newest First",
    "products.loadError": "Error loading products",
    "products.noProducts": "No products found.",
    "products.noProductsDesc": "Try adjusting your search or filters.",
    "products.inStock": "in stock",
    "products.similar": "Similar Products",

    "productDetail.title": "Product Detail",

    // Product Detail
    "product.idRequired": "Product ID is required.",
    "product.shareText": "Check out {title} on Sayiad!",
    "product.startAuction": "Start Auction",
    "product.addToCart": "Add to Cart",
    "product.quantity": "Quantity",
    "product.decreaseQty": "Decrease quantity",
    "product.increaseQty": "Increase quantity",
    "product.wishlist": "Wishlist",
    "product.viewAuction": "View Auction",
    "product.seller": "Seller",
    "product.noDescription": "No description provided.",
    "product.condition": "Condition",
    "product.location": "Location",
    "product.category": "Category",
    "product.stock": "Stock",
    "product.status": "Status",
    "product.brand": "Brand",
    "product.addedToCart": "Added to cart!",
    "product.contactSeller": "Contact Seller",
    "product.addedToWishlist": "Added to wishlist!",
    "product.removeFromWishlist": "Remove from Wishlist",
    "product.removedFromWishlist": "Removed from wishlist.",
    "product.draftRestored": "Draft restored",
    "product.discardDraft": "Discard draft",
    "product.invalidImageType": "Only JPG, PNG and WebP images are allowed.",
    "product.imageTooLarge": "Image must be under 5 MB.",
    "product.wishlistUpdated": "Wishlist updated!",
    "wishlist.confirmRemove": "Remove from wishlist?",
    "wishlist.confirmRemoveDesc":
      "This item will be removed from your wishlist.",
    "product.statusAvailable": "Available",
    "product.statusSold": "Sold",
    "product.statusDraft": "Draft",
    "product.statusRejected": "Rejected",
    "product.statusSuspended": "Suspended",
    "product.statusPendingReview": "Pending Review",
    "product.rating": "Rating",
    "product.comment": "Comment",
    "product.productId": "Product ID",

    // Auctions
    "auctions.title": "Auctions",
    "auctions.metaTitle": "Live Fish Auctions",
    "auctions.metaDesc":
      "Join live fish auctions on Sayiad. Bid on fresh catches.",
    "auctions.search": "Search auctions...",
    "auctions.allStatus": "All Status",
    "auctions.active": "Active",
    "auctions.finished": "Finished",
    "auctions.cancelled": "Cancelled",
    "auctions.loadError": "Error loading auctions",
    "auctions.noAuctionsDesc":
      "No auctions match the current filters. Browse products or check finished auctions.",

    // Auction Detail
    "auction.currentBid": "Current Bid",
    "auction.startingPrice": "Starting Price",
    "auction.reservePrice": "Reserve Price",
    "auction.bidIncrement": "Minimum Increment",
    "auction.totalBids": "Total Bids",
    "auction.start": "Start",
    "auction.end": "End",
    "auction.winner": "Winner",
    "auction.placeBid": "Place Bid",
    "auction.placingBid": "Placing...",
    "auction.bidHistory": "Bid History",
    "auction.noBids": "No bids yet. Be the first!",
    "auction.bidPlaced": "Bid placed successfully!",
    "auction.invalidBid": "Please enter a valid bid amount.",
    "auction.ended": "Ended",
    "auction.outbid": "You've been outbid!",
    "auction.endingSoon": "Ending soon!",
    "auction.newBid": "New bid placed!",
    "auction.startAuction": "Start Auction",
    "auction.startNew": "Start New Auction",
    "auction.bidCustomerOnly": "Only customers can place bids.",
    "auction.loginToBid": "Login as a customer to place bids.",
    "auction.autoBid": "Auto bid",
    "auction.autoBidMaxRequired":
      "Enter a max auto-bid amount higher than your bid.",
    "auction.reserveConfirmationPending": "Reserve price was not met. The seller must accept or reject the highest bid.",
    "auction.confirmBefore": "Respond before",
    "auction.acceptBid": "Accept bid",
    "auction.rejectBid": "Reject bid",
    "auction.acceptReserveTitle": "Accept highest bid?",
    "auction.acceptReserveConfirm": "Accept this bid and settle the auction.",
    "auction.rejectReserveTitle": "Reject highest bid?",
    "auction.rejectReserveConfirm": "Reject this bid, cancel the auction, and release held funds.",
    "auction.reserveAccepted": "Bid accepted. The auction was settled.",
    "auction.reserveRejected": "Bid rejected. The auction was cancelled.",
    "auction.statusActive": "Active",
    "auction.statusFinished": "Finished",
    "auction.statusCancelled": "Cancelled",
    "auction.statusScheduled": "Scheduled",
    "auction.statusPendingSellerConfirmation": "Pending seller confirmation",
    "auction.statusConfirmationExpired": "Confirmation expired",

    // Cart
    "cart.title": "Your Cart",
    "cart.clear": "Clear Cart",
    "cart.product": "Product",
    "cart.price": "Price",
    "cart.quantity": "Quantity",
    "cart.subtotal": "Subtotal",
    "cart.total": "Total",
    "cart.checkout": "Proceed to Checkout",
    "checkout.title": "Checkout",
    "common.amountPlaceholder": "0.00",
    "cart.empty": "Your cart is empty",
    "cart.removeItemConfirm": "Are you sure you want to remove this item?",
    "cart.emptyDesc": "Browse products to add items to your cart.",
    "cart.browseProducts": "Browse Products",
    "cart.cleared": "Cart cleared!",
    "cart.itemRemoved": "Item removed from cart.",
    "cart.placeOrder": "Place Order",
    "cart.insufficientWallet":
      "Insufficient demo wallet balance. Please add demo balance first.",
    "cart.placingOrder": "Placing Order...",
    "cart.shippingAddress": "Shipping Address",
    "cart.city": "City",
    "cart.addressLine": "Address",
    "cart.paymentMethod": "Payment Method",
    "cart.demoPaymentNotice":
      "Demo payment only: this checkout uses simulated wallet/test balance. No real money is charged.",
    "cart.creditCard": "Simulated Credit Card",
    "cart.cashOnDelivery": "Simulated Cash on Delivery",
    "cart.requiredFields": "Full Name, Phone, Address, and City are required.",
    "cart.orderSuccess": "Order placed successfully!",
    "cart.orderSuccessDesc":
      "Your order has been placed. Redirecting to dashboard...",
    "cart.backToCart": "Back to Cart",
    "cart.removeItemTitle": "Remove Item",
    "cart.clearConfirm": "Are you sure you want to clear your entire cart?",
    "cart.orderError": "Failed to place order",
    "cart.insufficientStock": "Only {stock} available",
    "cart.quantityUpdateFailed": "Could not update quantity. Please try again.",
    "cart.maxReached": "MAX",

    // Dashboard
    "dash.title": "Dashboard",
    "dash.tabsLabel": "Dashboard tabs",
    "dash.overview": "Overview",
    "dash.orders": "Orders",
    "dash.products": "Products",
    "dash.productsNotAvailable": "Not available for your role",
    "dash.auctions": "Auctions",
    "dash.wishlist": "Wishlist",
    "dash.sellerDashboard": "Seller Dashboard",
    "dash.addresses": "Addresses",
    "dash.notifications": "Notifications",
    "dash.profile": "Profile",
    "dash.changePassword": "Change Password",
    "dash.welcome": "Welcome back",
    "dash.role": "Role",
    "dash.totalOrders": "Total orders placed",
    "dash.yourProducts": "Your listed products",
    "dash.pendingReviews": "Pending Reviews",
    "dash.productsAwaitingReview": "Products awaiting review",
    "dash.viewAdminPanel": "Admin Panel →",
    "dash.totalUsers": "Total Users",
    "dash.registeredUsers": "Registered users",
    "dash.noOrders": "No orders yet.",
    "dash.noProducts": "You haven't listed any products yet.",
    "dash.emptyWishlist": "Your wishlist is empty.",
    "dash.noNotifications": "No notifications.",
    "dash.updateProfile": "Update Profile",
    "dash.updating": "Updating...",
    "dash.profileUpdated": "Profile updated successfully!",
    "dash.currentPassword": "Current Password",
    "dash.newPassword": "New Password",
    "dash.changePwBtn": "Change Password",
    "dash.changing": "Changing...",
    "dash.passwordChanged": "Password changed successfully!",
    "dash.orderNum": "Order #",
    "dash.date": "Date",
    "dash.view": "View",
    "dash.myProducts": "My Products",
    "profile.title": "My Profile",
    "profile.uploadPhoto": "Click to upload photo",
    "profile.imageTooLarge": "Image must be under 5 MB.",
    "profile.invalidImageType": "Choose a JPG, PNG, or WebP image.",
    "profile.invalidImageContent": "The selected file is not a valid supported image.",
    "profile.uploadNoUrl": "Upload did not return a URL.",
    "profile.photoUpdated": "Profile photo updated!",
    "profile.photoRemoved": "Profile photo removed.",
    "profile.confirmDeleteTitle": "Delete photo",
    "profile.confirmDeletePhoto":
      "Are you sure you want to remove your profile photo?",

    // Verify Email
    "verify.title": "Verify Email",
    "verify.success": "Email verified successfully! You can now login.",
    "verify.error": "Verification failed. The link may be expired or invalid.",
    "verify.redirecting": "Redirecting to login...",
    "verify.loggingIn": "Logging you in automatically...",
    "verify.invalidLink": "Invalid verification link.",
    "verify.waitingTitle": "Check your email",
    "verify.waitingDesc": "We sent a verification link to",
    "verify.alreadyVerified": "I already verified",
    "verify.useOtherEmail": "Use a different email",
    "verify.successTitle": "Email verified!",
    "verify.successDesc": "You are being logged in...",
    "verify.resend": "Resend Verification Email",
    "verify.resending": "Sending...",
    "verify.resendSuccess": "Verification email sent! Check your inbox.",
    "verify.resendError":
      "Failed to resend verification email. Please try again.",

    // Forgot password token entry
    "auth.tokenPlaceholder": "Paste the code or link from your email",
    "auth.tokenHint":
      "Paste the full email link — we'll extract the code automatically",

    // Shipping
    "shipping.title": "Shipping Addresses",
    "shipping.addNew": "Add New Address",
    "shipping.fullName": "Full Name",
    "shipping.phone": "Phone",
    "shipping.city": "City",
    "shipping.governorate": "Governorate",
    "shipping.addressLine": "Address Line",
    "shipping.postalCode": "Postal Code",
    "shipping.save": "Save Address",
    "shipping.saving": "Saving...",
    "shipping.delete": "Delete",
    "shipping.noAddresses": "No shipping addresses yet.",
    "shipping.savedAddresses": "Saved Addresses",
    "shipping.saved": "Address saved!",
    "shipping.deleted": "Address deleted!",
    "shipping.confirmDelete": "Delete this address?",
    "shipping.confirmDeleteDesc": "This address will be permanently deleted.",
    "shipping.error": "Failed to save address.",

    "auctionDetail.title": "Auction Detail",
    "privacy.title": "Privacy Policy",
    "terms.title": "Terms & Conditions",

    // Seller Profile
    "seller.title": "Seller Profile",
    "seller.myProfile": "My Seller Profile",
    "seller.storeName": "Store Name",
    "seller.description": "Description",
    "seller.contactEmail": "Contact Email",
    "seller.contactPhone": "Contact Phone",
    "seller.location": "Location",
    "seller.save": "Save Profile",
    "seller.saving": "Saving...",
    "seller.create": "Create Seller Profile",
    "seller.saved": "Profile saved!",
    "seller.notFound": "Seller profile not found.",
    "seller.rating": "Rating",
    "seller.totalSales": "Total Sales",
    "seller.totalProducts": "Total Products",
    "seller.recentOrders": "Recent Orders",
    "seller.noProfile": "You haven't created a seller profile yet.",
    "seller.becomeSeller": "Become a Seller",
    "seller.memberSince": "Member Since",
    "seller.listings": "Listings",
    "seller.setupRequired": "Set up your seller profile",
    "seller.setupDesc": "Complete your seller profile before listing products.",

    // Product Management
    "product.create": "Add Product",
    "product.edit": "Edit Product",
    "product.title": "Title",
    "product.description": "Description",
    "product.price": "Price",
    "product.new": "New",
    "product.used": "Used",
    "product.conditionNew": "New",
    "product.conditionUsed": "Used",
    "product.images": "Images",
    "product.uploadImage": "Upload Image",
    "product.uploading": "Uploading...",
    "product.save": "Save Product",
    "product.saving": "Saving...",
    "product.saved": "Product saved!",
    "product.deleted": "Product deleted.",
    "product.deleteConfirm": "Delete this product? This cannot be undone.",
    "product.deleteImage": "Remove",
    "product.chooseImage": "Choose Image",
    "product.imageError": "Failed to upload image.",
    "product.addAnother": "Add Another",

    // Admin
    "admin.title": "Admin Panel",
    "admin.users": "Users",
    "admin.reports": "Reports",
    "admin.products": "Products",
    "admin.orders": "Orders",
    "admin.categories": "Categories",
    "admin.plans": "Plans",
    "admin.id": "ID",
    "admin.name": "Name",
    "admin.email": "Email",
    "admin.role": "Role",
    "admin.status": "Status",
    "admin.active": "Active",
    "admin.inactive": "Inactive",
    "admin.suspended": "Suspended",
    "admin.suspend": "Suspend",
    "admin.activate": "Activate",
    "admin.toggleStatus": "Toggle Status",
    "admin.reportReason": "Reason",
    "admin.reportStatus": "Status",
    "admin.resolve": "Resolve",
    "admin.dismiss": "Dismiss",
    "admin.addCategory": "Add Category",
    "admin.categoryName": "Category Name",
    "admin.categoryDesc": "Description",
    "admin.noAccess": "You do not have admin access.",
    "admin.userToggled": "User status updated!",
    "admin.confirmDeleteCategory": "Delete category?",
    "admin.confirmDeleteCategoryDesc":
      "All products in this category may be affected.",
    "admin.categoryDeleted": "Category deleted",
    "admin.categoryAdded": "Category added",
    "admin.reportResolved": "Report resolved",
    "admin.reportType": "Type",
    "admin.reportTarget": "Target",
    "admin.reportMessage": "Message",
    "admin.reportAdminNote": "Admin Note",
    "admin.resolvedAt": "Resolved At",
    "admin.pendingFreezes": "Pending Freezes",
    "admin.pendingReports": "Pending Reports",
    "admin.dismissReport": "Dismiss Report",
    "admin.dismissReportConfirm": "Dismiss this report without action?",
    "admin.reportDismissed": "Report dismissed.",
    "admin.productStatusUpdated": "Product status updated",
    "admin.planCreated": "Plan created",
    "admin.planUpdated": "Plan updated",
    "admin.planDeleted": "Plan deleted",
    "admin.confirmDeletePlan": "Delete plan?",
    "admin.confirmDeletePlanDesc": "This cannot be undone.",
    "admin.revenue": "Revenue",
    "admin.review": "Review",
    "admin.approve": "Approve",
    "admin.reject": "Reject",
    "admin.rejectionReason": "Rejection Reason",
    "admin.confirmApprove": "Approve this product?",
    "admin.confirmApproveDesc":
      "This will make the product visible to all users.",
    "admin.productApproved": "Product approved!",
    "admin.productRejected": "Product rejected",
    "admin.noPendingReviews": "No products pending review",
    "admin.productsAwaiting": "Products awaiting your review",
    "admin.platformBalance": "Platform Balance",
    "admin.totalFees": "Total Fees Collected",
    "admin.feeIncome": "Fee Income",
    "admin.addPlan": "Add Plan",
    "admin.editPlan": "Edit Plan",
    "admin.addSubscriptionPlan": "Add Subscription Plan",
    "admin.planDescription": "Description",
    "admin.planPrice": "Price",
    "admin.planPriceEgp": "Price (EGP)",
    "admin.planAuctions": "Auctions",
    "admin.planBids": "Bids",
    "admin.planRequests": "Requests",
    "admin.planMaxAuctions": "Max Auctions/Month",
    "admin.planMaxBids": "Max Bids/Month",
    "admin.planMaxRequests": "Max Requests/Month",
    "admin.planSortOrder": "Sort Order",
    "admin.confirmSuspendUser": "Suspend user?",
    "admin.confirmSuspendUserDesc": "Suspend {user}? They will lose access until reactivated.",
    "admin.confirmActivateUser": "Activate user?",
    "admin.confirmActivateUserDesc": "Activate {user} and restore account access.",
    "admin.confirmApproveRole": "Approve role request?",
    "admin.confirmApproveRoleDesc": "Approve {role} for {user}?",
    "admin.confirmRejectRole": "Reject role request?",
    "admin.confirmRejectRoleDesc": "Reject the {role} request for {user}?",

    // Order Detail
    "order.title": "Order Details",
    "order.items": "Items",
    "order.total": "Total",
    "order.status": "Status",
    "order.statusPending": "Pending",
    "order.statusPaid": "Paid",
    "order.statusShipped": "Shipped",
    "order.statusDelivered": "Delivered",
    "order.statusCancelled": "Cancelled",
    "order.date": "Date",
    "order.buyer": "Buyer",
    "order.seller": "Seller",
    "order.summary": "Order Summary",
    "order.shipping": "Shipping",
    "order.quantity": "Qty",
    "order.price": "Price",
    "order.subtotal": "Subtotal",
    "order.backToOrders": "Back to Orders",
    "order.notFound": "Order not found.",
    "order.cancel": "Cancel Order",
    "order.cancelling": "Cancelling...",
    "order.cancelled": "Order cancelled successfully!",
    "order.cancelConfirm":
      "Are you sure you want to cancel this order? This cannot be undone.",
    "order.cancelError": "Failed to cancel order.",
    "order.statusReturnRequested": "Return Requested",
    "order.statusReturned": "Returned",
    "order.deliveredAt": "Delivered At",
    "order.returnReason": "Return Reason",
    "order.requestReturn": "Request Return",
    "order.returnConfirm": "Are you sure you want to request a return? The seller will be notified.",
    "order.returnPending": "A return request is pending. Waiting for admin approval.",
    "order.returnRequested": "Return requested successfully!",
    "order.returnError": "Failed to process return request.",
    "order.approveReturn": "Approve Return",
    "order.approveReturnConfirm": "Approve this return request? The buyer will be refunded.",
    "order.returnApproved": "Return approved and buyer refunded.",
    "order.rejectReturn": "Reject Return",
    "order.rejectReturnReason": "Reason for rejecting the return:",
    "order.rejectReturnConfirm": "Reject this return request?",
    "order.returnRejected": "Return request rejected.",
    // Reviews
    "review.title": "Reviews",
    "review.writeReview": "Write a Review",
    "review.rating": "Rating",
    "review.comment": "Comment",
    "review.submit": "Submit Review",
    "review.submitting": "Submitting...",
    "review.submitted": "Review submitted!",
    "review.noReviews": "No reviews yet.",
    "review.delete": "Delete",
    "review.rateProduct": "Rate this product",
    "review.stars": "stars",
    "review.sort": "Sort reviews",
    "review.highestRated": "Highest Rated",
    "review.lowestRated": "Lowest Rated",

    // Notifications
    "notif.title": "Notifications",
    "notif.markRead": "Mark Read",
    "notif.markAllRead": "Mark All Read",
    "notif.markedAllRead": "All marked as read",
    "notif.noNotifications": "No notifications",
    "notif.unreadCount": "unread",

    // Common
    "common.loading": "Loading...",
    "common.pageNotFound": "Page Not Found",
    "common.pageNotFoundDesc": "The page you are looking for does not exist.",
    "common.goHome": "Go Home",
    "common.refresh": "Refresh",
    "common.offline": "You are offline. Some features may be unavailable.",
    "common.backOnline": "Back online!",
    "common.somethingWentWrong": "Something went wrong",
    "common.errorFallbackDesc":
      "An unexpected error occurred. Please try refreshing the page.",
    "common.error": "Error",
    "common.endsIn": "left",
    "common.bids": "bids",
    "common.start": "Start",
    "products.showingCount": "Showing {count} of {total} products",
    "common.next": "Next",
    "common.previous": "Previous",
    "common.N/A": "N/A",
    "common.search": "Search",
    "common.select": "Select",
    "common.required": "Required",
    "common.optional": "Optional",
    "common.linkCopied": "Link copied to clipboard!",
    "common.refreshing": "Refreshing...",
    "common.pullToRefresh": "Pull to refresh",
    "common.confirm": "Confirm",
    "common.hot": "HOT",
    "common.qty": "Qty",
    "common.retry": "Retry",
    "common.days": "days",
    "common.hours": "hrs",
    "common.minutes": "min",
    "common.seconds": "sec",
    "common.page": "Page",
    "common.clearFilters": "Clear Filters",
    "common.loadFailed": "Failed to load",
    "common.remove": "Remove",
    "common.delete": "Delete",
    "common.save": "Save",
    "common.recent": "Recent",
    "common.recentlyViewed": "Recently Viewed",
    "common.quickLinks": "Quick Links",
    "common.cancel": "Cancel",
    "common.close": "Close",
    "common.dismiss": "Dismiss",
    "common.breadcrumb": "Breadcrumb",
    "common.product": "Product",
    "common.category": "Category",
    "common.back": "Back",
    "common.title": "Title",
    "common.name": "Name",
    "common.tier": "Tier",
    "common.status": "Status",
    "common.actions": "Actions",
    "common.view": "View",
    "common.free": "Free",
    "common.secureCheckout": "Secure Checkout",
    "common.easyReturns": "Easy Returns",
    "auth.loginRequired": "Please log in to continue.",

    // Auction Requests
    "auctionRequests.title": "Auction Requests",
    "auctionRequests.submit": "Submit Auction Request",
    "auctionRequests.myRequests": "My Requests",
    "auctionRequests.productTitle": "Product Title",
    "auctionRequests.productDescription": "Description",
    "auctionRequests.estimatedValue": "Estimated Value (EGP)",
    "auctionRequests.quantityKg": "Quantity (kg)",
    "auctionRequests.fishType": "Fish Type",
    "auctionRequests.catchLocation": "Catch Location",
    "auctionRequests.catchDate": "Catch Date",
    "auctionRequests.submitting": "Submitting...",
    "auctionRequests.submitted": "Request submitted successfully!",
    "auctionRequests.status": "Status",
    "auctionRequests.createdAt": "Submitted At",
    "auctionRequests.rejectionReason": "Reason",
    "auctionRequests.pending": "Pending",
    "auctionRequests.approved": "Approved",
    "auctionRequests.rejected": "Rejected",
    "auctionRequests.noRequests": "No auction requests yet",
    "auctionRequests.noRequestsDesc":
      "Submit a request to have your catch featured in an auction.",
    "auctionRequests.requestAuction": "Request Auction",
    "auctionRequests.imageUrl": "Product Image URL",
    "auctionRequests.imageUrlHelp":
      "Optional — add a URL for the product image",

    // Auction Requests Review
    "auctionRequestsReview.title": "Review Auction Requests",
    "auctionRequestsReview.pending": "Pending",
    "auctionRequestsReview.approved": "Approved",
    "auctionRequestsReview.rejected": "Rejected",
    "auctionRequestsReview.all": "All",
    "auctionRequestsReview.approve": "Approve",
    "auctionRequestsReview.reject": "Reject",
    "auctionRequestsReview.approving": "Approving...",
    "auctionRequestsReview.rejecting": "Rejecting...",
    "auctionRequestsReview.approvedSuccess": "Request approved successfully!",
    "auctionRequestsReview.rejectedSuccess": "Request rejected successfully!",
    "auctionRequestsReview.rejectionReason": "Rejection Reason",
    "auctionRequestsReview.rejectionReasonPlaceholder":
      "Explain why this request is being rejected",
    "auctionRequestsReview.noPending": "No pending auction requests",
    "auctionRequestsReview.noPendingDesc": "All requests have been reviewed.",
    "auctionRequestsReview.confirmApprove":
      "Are you sure you want to approve this request?",
    "auctionRequestsReview.confirmReject":
      "Are you sure you want to reject this request?",
    "auctionRequestsReview.fisherman": "Fisherman",
    "auctionRequestsReview.actions": "Actions",

    // Auctioneer Analytics
    "analytics.statusDistribution": "Auctions Distribution",
    "analytics.title": "Auctioneer Analytics",
    "analytics.totalAuctions": "Total Auctions",
    "analytics.activeAuctions": "Active Auctions",
    "analytics.finishedAuctions": "Finished Auctions",
    "analytics.totalBids": "Total Bids",
    "analytics.totalRevenue": "Total Revenue",
    "analytics.totalFees": "Total Fees Earned",
    "analytics.feeIncome": "Fee Income",
    "analytics.availableInWallet": "in wallet",
    "analytics.recentAuctions": "Recent Auctions",
    "analytics.startingPrice": "Starting Price",
    "analytics.currentPrice": "Current Price",
    "analytics.bidCount": "Bids",
    "analytics.endTime": "End Time",
    "analytics.noData": "No auction data available",

    // Scheduling
    "scheduling.startTime": "Auction Start Time",
    "scheduling.endTime": "Auction End Time",
    "scheduling.startTimeRequired": "Please select an auction start time",
    "scheduling.scheduleAuction": "Schedule Auction",
    "scheduling.scheduling": "Scheduling...",
    "scheduling.info":
      "Set the start time and optionally the end time. Default end time is 7 days from start.",

    // Wallet
    "wallet.title": "My Wallet",
    "wallet.pageLabel": "Wallet",
    "wallet.actions": "Wallet actions",
    "wallet.loadingTransactions": "Loading transactions",
    "wallet.closeAction": "Close wallet action dialog",
    "wallet.transactionHistory": "Transaction history",
    "wallet.minimumDeposit": "Minimum EGP 10",
    "wallet.balance": "Total Balance",
    "wallet.held": "Held Balance",
    "wallet.available": "Available Test Balance",
    "wallet.deposit": "Add Demo Balance",
    "wallet.withdraw": "Withdraw",
    "wallet.enterAmount": "Enter amount",
    "wallet.invalidAmount": "Please enter a valid positive amount",
    "wallet.amountTooLarge": "Maximum deposit is EGP 100,000",
    "wallet.invalidDecimal": "Only 2 decimal places allowed",
    "wallet.depositSuccess": "Demo balance added successfully!",
    "wallet.depositError": "Could not add demo balance. Please try again.",
    "wallet.transactions": "Transactions",
    "wallet.date": "Date",
    "wallet.type": "Type",
    "wallet.description": "Description",
    "wallet.amount": "Amount",
    "wallet.noTransactions": "No transactions yet",
    "wallet.loadError": "Failed to load wallet data.",
    "wallet.platformFee": "Platform Fee",
    "wallet.subPayment": "Demo Subscription Payment",
    "wallet.auctionPayment": "Demo Auction Payment",
    "wallet.auctionPayout": "Demo Auction Payout",
    "wallet.readOnly": "Wallet is in read-only mode",
    "wallet.paymentNote":
      "Add demo balance instantly for testing. This does not represent real money.",
    "wallet.topUpTitle": "Add Demo Balance",
    "wallet.topUpHelp":
      "This adds test balance instantly. No real money transfer, bank payout, or withdrawal is processed.",
    "wallet.amountLabel": "Amount (EGP)",
    "wallet.withdrawTitle": "Withdraw Balance",
    "wallet.withdrawHelp": "Enter the amount to withdraw from your available wallet balance.",
    "wallet.withdrawPlaceholder": "Enter a positive amount",
    "wallet.withdrawAmountError": "Withdrawal amount must be positive.",
    "wallet.withdrawSuccess": "Balance withdrawn successfully!",
    "wallet.confirmWithdraw": "Withdraw Balance",
    "wallet.noTransactionsDesc": "Your transaction history will appear here.",
    "wallet.colStatus": "Status",
    "wallet.metaDesc": "Manage your Sayiad wallet balance and transactions.",
    "wallet.minAmountError": "Please enter a valid amount (minimum EGP 10)",
    "wallet.maxAmountError": "Maximum top-up is EGP 50,000",
    "wallet.topUpSuccess": "Demo balance added successfully!",
    "wallet.confirmTopUp": "Add Demo Balance",

    // Subscriptions
    "subscriptions.title": "Subscription Plans",
    "subscriptions.currentPlan": "My Current Plan",
    "subscriptions.noPlan": "No Active Plan",
    "subscriptions.free": "Free",
    "subscriptions.basic": "Basic",
    "subscriptions.freeDesc":
      "Basic access to browse and explore the marketplace",
    "subscriptions.premium": "Premium",
    "subscriptions.premiumDesc":
      "Full access with priority support and extra features",
    "subscriptions.pro": "Professional",
    "subscriptions.proLabel": "Pro",
    "subscriptions.proDesc":
      "Everything in Premium plus unlimited auctions and analytics",
    "subscriptions.enterprise": "Enterprise",
    "subscriptions.monthly": "Monthly",
    "subscriptions.yearly": "Yearly",
    "subscriptions.perMonth": "/mo",
    "subscriptions.perYear": "/yr",
    "subscriptions.daysLeft": "{count} days left",
    "subscriptions.endsToday": "Ends today",
    "subscriptions.expired": "Expired",
    "subscriptions.popular": "Most Popular",
    "subscriptions.feature1": "Browse products and auctions",
    "subscriptions.feature2": "Place bids and make purchases",
    "subscriptions.feature3": "Create seller profile",
    "subscriptions.feature4": "Priority customer support",
    "subscriptions.feature5": "Advanced analytics dashboard",
    "subscriptions.feature6": "Unlimited auction requests",
    "subscriptions.feature7": "Featured listings",
    "subscriptions.upgrade": "Upgrade with Test Balance",
    "subscriptions.current": "Current",
    "subscriptions.learnMore": "Learn More",
    "subscriptions.upgradeSuccess": "Demo subscription upgraded successfully!",
    "subscriptions.upgradeError": "Could not upgrade subscription.",
    "subscriptions.invalidTier": "This plan cannot be upgraded right now.",
    "subscriptions.loadError": "Failed to load subscription data.",
    "subscriptions.customerHeading": "Unlock More Bids",
    "subscriptions.customerDesc":
      "Buy a subscription to bid on more auctions and get better deals",
    "subscriptions.auctioneerHeading": "Grow Your Business",
    "subscriptions.auctioneerDesc":
      "Buy a subscription to accept and manage more auctions with advanced features",
    "subscriptions.sellerHeading": "Upgrade Your Experience",
    "subscriptions.sellerDesc":
      "Buy a subscription to unlock more listings, auctions, and premium features",
    "subscriptions.insufficientFunds": "Insufficient Balance",
    "subscriptions.demoPaymentNote":
      "Subscription upgrades use demo wallet/test balance. No real payment reference or money transfer is processed.",

    // Footer
    "footer.tagline":
      "Egypt's premier fishing gear marketplace and auction platform.",
    "footer.marketplace": "Marketplace",
    "footer.account": "Account",
    "footer.legal": "Legal",
    "footer.sellOnSayiad": "Sell on Sayiad",
    "footer.myAddresses": "My Addresses",
    "footer.allRights": "All rights reserved.",
    "footer.madeWith": "Made for fishermen 🎣",
    "footer.copyright": "\u00a9 2026 Sayiad. All rights reserved.",

    // Admin
    "admin.activeAuctions": "Active Auctions",
    "admin.createFirstCategory":
      "Create your first category to organize products.",
    "admin.noCategories": "No categories found",
    "admin.noFees": "No fee transactions yet",
    "admin.noOrders": "No orders found",
    "admin.noReports": "No reports found",
    "admin.noUsers": "No users found",
    "admin.totalRevenue": "Total Revenue",
    "admin.totalUsers": "Total Users",
    "admin.pendingRoleRequests": "Pending Role Requests",
    "admin.roleApproved": "Role request approved!",
    "admin.roleRejected": "Role request rejected",
    "admin.rejectRoleRequest": "Reject Role Request",
    "admin.manageUserReviews": "Manage User Reviews",
    "admin.enterProductId": "Enter Product ID",
    "admin.noUserReviews": "No reviews found for this product",
    "admin.removeReview": "Remove Review",
    "admin.confirmRemoveReview": "Remove review by {userName}?",
    "admin.reviewRemovedNotified": "Review removed. Author has been notified.",

    // Auction
    "auction.anonymousBidder": "Anonymous Bidder",
    "auction.autoBidMaxPlaceholder": "Max bid amount",
    "auction.bidIrreversible": "This cannot be undone.",
    "auction.confirmBidMsg": "Place a bid of",
    "auction.confirmBidTitle": "Confirm Your Bid",
    "auction.item": "Auction Item",
    "auction.idRequired": "Auction ID is required.",
    "auction.userNumber": "User #",

    // Auctions
    "auctions.status": "Status",

    // Cart
    "cart.continueShopping": "Continue Shopping",

    // Common
    "common.edit": "Edit",
    "common.finish": "Finish",
    "common.processing": "Processing\u2026",
    "common.share": "Share",
    "common.showResults": "Show Results",
    "common.step": "Step",
    "common.loadMore": "Load More",
    "common.noDescription": "No description provided",
    "common.viewProfile": "View Profile",
    "common.kgUnit": "kg",
    "common.unknown": "?",

    // Notifications
    "notif.markAsRead": "Mark as read",

    // Order
    "order.confirmed": "Confirmed",
    "order.delivered": "Delivered",
    "order.method": "Method",
    "order.paymentInfo": "Payment",
    "order.processing": "Processing",
    "order.reorder": "Shop Again",
    "order.shipped": "Shipped",
    "order.shippingInfo": "Shipping",
    "order.tax": "Tax",
    "order.track": "View Order Details",

    // Products
    "products.category": "Category",
    "products.condition": "Condition",

    // Profile
    "profile.completion": "Profile Completion",

    // Shipping
    "shipping.editAddress": "Edit Address",
    "shipping.locationPreview": "Location Preview",
    "shipping.updated": "Address updated successfully",

    // Subscriptions
    "subscriptions.noPlans": "No plans found",
    "subscriptions.noPlansDesc":
      "Subscription plans are not available at the moment.",

    // Tour
    "tour.navDesc":
      "Navigate seamlessly between your overview, products, order tracking, and profile.",
    "tour.navTitle": "Dashboard Navigation",
    "tour.ordersDesc":
      "Monitor your transactions, purchase records, and shipping updates dynamically.",
    "tour.ordersTitle": "Track Orders",
    "tour.productsDesc":
      "Fisherman and sellers can easily publish or update listed catch and auction items here.",
    "tour.productsTitle": "Manage Listings",
    "tour.welcomeToSayiad": "Ready to use Sayiad! Enjoy your experience.",

    // Pre-UI translation coverage
    "admin.remove": "Remove",
    "auctions.bids": "bids",
    "auctions.ended": "Ended",
    "auctions.ending": "Ending soon",
    "auctions.noAuctions": "No auctions found",
    "auth.acceptTerms": "I accept the",
    "auth.codePlaceholder": "Enter verification code",
    "auth.codeSent": "We sent a verification code to your email.",
    "auth.confirmPasswordPlaceholder": "Confirm your password",
    "auth.emailVerified": "Email verified successfully.",
    "auth.enterCode": "Enter verification code",
    "auth.enterEmail": "Enter your email address",
    "auth.enterNewPassword": "Enter your new password",
    "auth.error": "Authentication error",
    "auth.haveAccount": "Already have an account?",
    "auth.invalidCode": "Invalid verification code.",
    "auth.loginDesc": "Sign in to continue to your Sayiad account.",
    "auth.invalidCredentials": "Incorrect email or password.",
    "auth.loginError": "Unable to sign in. Please check your details and try again.",
    "auth.mustAcceptTerms": "You must accept the terms before creating an account.",
    "auth.passwordPlaceholder": "Enter your password",
    "auth.redirectingToLogin": "Redirecting to login...",
    "auth.registerDesc": "Create your Sayiad account and choose your marketplace role.",
    "auth.registerError": "Unable to create your account. Please try again.",
    "auth.resendIn": "You can request another code in",
    "auth.sendCode": "Send Code",
    "auth.verificationFailed": "Email verification failed.",
    "auth.verifyPrompt": "Please verify your email before logging in.",
    "auth.welcomeBack": "Welcome Back",
    "common.action": "Action",
    "common.auction": "Auction",
    "common.date": "Date",
    "common.errorOccurred": "An error occurred.",
    "common.hidePassword": "Hide password",
    "common.of": "of",
    "common.showPassword": "Show password",
    "common.tryAgain": "Try Again",
    "common.yearsOld": "years old",
    "error_service_unavailable": "Service Unavailable",
    "error_service_unavailable_desc": "The service is temporarily unavailable. Please try again shortly.",
    "footer.privacy": "Privacy Policy",
    "footer.quickLinks": "Quick Links",
    "footer.rights": "All rights reserved.",
    "footer.shipping": "Shipping",
    "footer.support": "Support",
    "footer.terms": "Terms",
    "nav.profile": "Profile",
    "profile.removePhoto": "Remove photo",
    "register.auctioneer": "Auctioneer",
    "register.auctioneerDesc": "Manage auctions and review auction requests.",
    "register.baitSeller": "Bait Seller",
    "register.baitSellerDesc": "Sell bait and fishing supplies.",
    "register.customer": "Customer",
    "register.customerDesc": "Shop products and bid on auctions.",
    "register.fisherman": "Fisherman",
    "register.fishermanDesc": "Sell your catch and request auctions.",
    "retry": "Retry",
    "subscriptions.baitSellerDesc": "Access seller tools for bait and supplies.",
    "subscriptions.baitSellerHeading": "Grow Your Bait Shop",
    "subscriptions.fishermanDesc": "Unlock selling tools for catch listings and auction requests.",
    "subscriptions.fishermanHeading": "Grow Your Fishing Business",
    "validation.invalidEmail": "Please enter a valid email address.",
    "validation.invalidPhone": "Please enter a valid phone number.",
    "validation.passwordsNotMatch": "Passwords do not match.",

    // Validation
    "validation.invalid": "Invalid value",
    "validation.minLength": "Value is too short",
    "validation.required": "This field is required",
    "validation.minAge": "Must be at least {minAge} years old",
  },

  ar: {
    // Navbar
    "nav.home": "الرئيسية",
    "home.title": "الرئيسية",
    "home.metaDesc": "سوق صيد الأسماك الأول في مصر.",
    "nav.products": "المنتجات",
    "nav.auctions": "المزادات",
    "nav.cart": "عربة التسوق",
    "nav.login": "تسجيل الدخول",
    "nav.register": "إنشاء حساب",
    "nav.dashboard": "لوحة التحكم",
    "nav.myOrders": "طلباتي",
    "nav.wallet": "المحفظة",
    "nav.myProducts": "منتجاتي",
    "nav.wishlist": "المفضلة",
    "nav.logout": "تسجيل الخروج",
    "nav.searchPlaceholder": "ابحث عن سمك، معدات، مزادات\u2026",

    // Home
    "tour.welcome":
      "تصفح أدوات الصيد الفاخرة من أفضل العلامات التجارية. ابحث عن القضبان، البكرات، الطُعم، والمزيد.",
    "tour.products":
      "استكشف كتالوجنا مع البحث المتadvanced والمرشحات للعثور على ما تحتاجه بالضبط.",
    "tour.auctions":
      "شارك في المزادات الفورية على معدات الصيد الحصرية. احصل على صفقات رائعة!",
    "home.welcome": "مرحباً بك في صياد",
    "home.subtitle":
      "السوق المتكامل لمعدات وأدوات الصيد. اشترِ، بع، وشارك في المزادات على منتجات الصيد المتميزة.",
    "home.browseProducts": "تصفح المنتجات",
    "home.viewAuctions": "عرض المزادات",
    "home.qualityGear": "معدات عالية الجودة",
    "home.qualityGearDesc": "معدات صيد متميزة من بائعين موثوقين",
    "home.liveAuctions": "مزادات حية",
    "home.liveAuctionsDesc":
      "شارك بالمزايدة في الوقت الفعلي على منتجات الصيد الحصرية",
    "home.fastShipping": "شحن سريع",
    "home.fastShippingDesc": "توصيل موثوق إلى باب منزلك",
    "home.securePayments": "مدفوعات آمنة",
    "home.securePaymentsDesc": "معالجة معاملات آمنة ومحمية",
    "home.latestProducts": "أحدث المنتجات",
    "home.activeAuctions": "المزادات النشطة",
    "home.viewAll": "عرض الكل",
    "home.noProducts": "لا توجد منتجات بعد",
    "home.noAuctions": "لا توجد مزادات نشطة",
    "home.loadError": "تعذر تحميل المنتجات.",

    // Auth
    "auth.login": "تسجيل الدخول",
    "login.title": "تسجيل الدخول",
    "auth.logoutTitle": "تسجيل الخروج",
    "auth.logoutConfirm": "هل أنت متأكد أنك تريد تسجيل الخروج؟",
    "auth.email": "البريد الإلكتروني",
    "auth.password": "كلمة المرور",
    "auth.signIn": "تسجيل الدخول",
    "auth.signingIn": "جارٍ تسجيل الدخول...",
    "auth.noAccount": "ليس لديك حساب؟",
    "auth.register": "إنشاء حساب",
    "register.title": "إنشاء حساب",
    "auth.fullName": "الاسم الكامل",
    "auth.phone": "رقم الهاتف",
    "auth.birthdate": "تاريخ الميلاد",
    "auth.role": "الدور",
    "auth.customer": "عميل",
    "auth.fisherman": "صياد",
    "auth.baitSeller": "بائع طعم",
    "auth.licenseNumber": "رقم ترخيص الصيد",
    "auth.licenseRequired": "رقم ترخيص الصيد مطلوب لتسجيل الصياد.",
    "auth.auctioneer": "مزاد علني",
    "auth.createAccount": "إنشاء حساب",
    "auth.creatingAccount": "جارٍ إنشاء الحساب...",
    "auth.hasAccount": "لديك حساب بالفعل؟",
    "register.auctioneerNotice":
      "حسابات المزاد العلني تتطلب موافقة المشرف قبل التفعيل. ستتمكن من استخدام ميزات العميل الأساسية فوراً وستتلقى إشعاراً عند الموافقة.",
    "register.pendingUpgrade":
      "تم تقديم طلب دور المزاد العلني. يمكنك تسجيل الدخول الآن واستخدام ميزات العميل أثناء انتظار موافقة المشرف.",
    "register.goToLogin": "انتقال إلى تسجيل الدخول",
    "auth.registerSuccess":
      "تم إنشاء الحساب! تحقق من بريدك الإلكتروني للتحقق قبل تسجيل الدخول.",
    "auth.loginSuccess": "تم تسجيل الدخول بنجاح!",
    "auth.loggedOut": "تم تسجيل الخروج بنجاح.",
    "auth.showPassword": "إظهار كلمة المرور",
    "auth.hidePassword": "إخفاء كلمة المرور",
    "auth.fullNamePlaceholder": "John Doe",
    "auth.emailPlaceholder": "your@email.com",
    "auth.phonePlaceholder": "+1234567890",
    "auth.licensePlaceholder": "FL-123456",
    "auth.passwordRequirements": "متطلبات كلمة المرور",
    "auth.confirmPassword": "تأكيد كلمة المرور",
    "auth.passwordsDoNotMatch": "كلمات المرور غير متطابقة.",
    "auth.passwordMinLength": "يجب أن تكون كلمة المرور 8 أحرف على الأقل.",
    "auth.passwordRequiresUppercase":
      "يجب أن تحتوي على حرف كبير واحد على الأقل.",
    "auth.passwordRequiresLowercase":
      "يجب أن تحتوي على حرف صغير واحد على الأقل.",
    "auth.passwordRequiresDigit": "يجب أن تحتوي على رقم واحد على الأقل.",
    "auth.invalidEmail": "الرجاء إدخال بريد إلكتروني صحيح.",
    "auth.fieldRequired": "هذا الحقل مطلوب.",
    "auth.minAgeRequired": "يجب أن يكون عمرك {minAge} عامًا على الأقل.",
    "auth.invalidSpecialChar":
      "يجب أن تحتوي على رمز خاص واحد على الأقل (!@#$%^&*).",
    "auth.ageLabel": "العمر: {age} سنة",
    "auth.invalidAlphanumeric": "يجب أن يحتوي هذا الحقل على أحرف وأرقام فقط.",
    "auth.emailAlreadyRegistered": "هذا البريد الإلكتروني مسجل بالفعل.",
    "auth.passwordStrength.weak": "ضعيفة",
    "auth.passwordStrength.fair": "متوسطة",
    "auth.passwordStrength.good": "جيدة",
    "auth.passwordStrength.strong": "قوية",
    "auth.iAccept": "أوافق على",
    "auth.termsAndConditions": "الشروط والأحكام",
    "auth.and": "و",
    "auth.privacyPolicy": "سياسة الخصوصية",
    "auth.termsRequired": "يرجى الموافقة على الشروط والأحكام.",
    "auth.forgotPassword": "نسيت كلمة المرور؟",
    "auth.tooManyAttempts": "محاولات فاشلة كثيرة. انتظر {secs} ثوانٍ.",
    "auth.sendResetLink": "إرسال رابط إعادة التعيين",
    "auth.sendingResetLink": "جارٍ إرسال الرابط...",
    "auth.resetLinkSent":
      "تم إرسال رابط إعادة تعيين كلمة المرور إلى بريدك الإلكتروني.",
    "auth.verificationCode": "كود التحقق",
    "auth.verifyCode": "تحقق من الكود",
    "auth.didntGetCode": "لم يصلك الكود؟",
    "auth.resendCode": "إعادة إرسال الكود",
    "auth.resendLink": "إعادة إرسال الرابط",
    "auth.verifying": "جارٍ التحقق...",
    "auth.verificationTimeout":
      "انتهت مهلة التحقق. يرجى التحقق من بريدك الإلكتروني ومحاولة تسجيل الدخول.",
    "auth.veryWeak": "ضعيف جداً",
    "auth.weak": "ضعيف",
    "auth.fair": "مقبول",
    "auth.strong": "قوي",
    "auth.veryStrong": "قوي جداً",
    "auth.reqLength": "8+ أحرف",
    "auth.reqUpper": "حرف كبير",
    "auth.reqLower": "حرف صغير",
    "auth.reqNum": "رقم",
    "auth.reqSpecial": "رمز خاص",
    "auth.invalidToken": "الرمز غير صالح أو منتهي الصلاحية.",
    "auth.resetPassword": "إعادة تعيين كلمة المرور",
    "auth.newPassword": "كلمة المرور الجديدة",
    "auth.confirmNewPassword": "تأكيد كلمة المرور الجديدة",
    "auth.updatingPassword": "جارٍ تحديث كلمة المرور...",
    "auth.passwordResetSuccess":
      "تم تحديث كلمة المرور بنجاح! جاري التحويل لتسجيل الدخول...",
    "auth.emailNotVerified": "البريد الإلكتروني غير مُتحقق منه.",
    "auth.checkInbox": "يرجى التحقق من بريدك الوارد والنقر على رابط التحقق.",
    "auth.checkSpam": "لم تستلمه؟ تحقق من مجلد الرسائل غير المرغوب فيها.",
    "auth.resendVerification": "إعادة إرسال التحقق",
    "auth.verificationSent": "تم إرسال بريد التحقق!",
    "auth.sending": "جارٍ الإرسال...",
    "auth.sent": "تم الإرسال!",
    "auth.emailNotFound": "البريد الإلكتروني غير موجود",
    "auth.emailNotFoundRegister":
      "البريد الإلكتروني غير موجود. هل تريد التسجيل؟",

    // Products
    "products.gridView": "عرض شبكي",
    "products.listView": "عرض قائمة",
    "products.title": "المنتجات",
    "products.metaTitle": "منتجات الأسماك والمأكولات البحرية",
    "products.metaDesc": "تصفح الأسماك الطازجة والمأكولات البحرية في سوق صياد.",
    "products.search": "ابحث عن منتجات...",
    "products.allCategories": "جميع الفئات",
    "products.allConditions": "كل الحالات",
    "products.minPrice": "أقل سعر",
    "products.maxPrice": "أعلى سعر",
    "products.inStockOnly": "المتوفر فقط",
    "products.filters": "التصفية",
    "products.sort": "افتراضي",
    "products.priceLowHigh": "السعر: من الأقل للأعلى",
    "products.priceHighLow": "السعر: من الأعلى للأقل",
    "products.newest": "الأحدث أولاً",
    "products.loadError": "خطأ في تحميل المنتجات",
    "products.noProducts": "لم يتم العثور على منتجات.",
    "products.noProductsDesc": "حاول تعديل البحث أو التصفية.",
    "products.inStock": "متوفر",
    "products.similar": "منتجات مشابهة",

    "productDetail.title": "تفاصيل المنتج",

    // Product Detail
    "product.idRequired": "معرف المنتج مطلوب.",
    "product.shareText": "تحقق من {title} على سياد!",
    "product.startAuction": "بدء المزاد",
    "product.addToCart": "أضف إلى العربة",
    "product.quantity": "الكمية",
    "product.decreaseQty": "تقليل الكمية",
    "product.increaseQty": "زيادة الكمية",
    "product.wishlist": "المفضلة",
    "product.viewAuction": "عرض المزاد",
    "product.seller": "البائع",
    "product.noDescription": "لا يوجد وصف.",
    "product.condition": "الحالة",
    "product.location": "الموقع",
    "product.category": "الفئة",
    "product.stock": "المخزون",
    "product.status": "الحالة",
    "product.brand": "العلامة التجارية",
    "product.addedToCart": "تمت الإضافة إلى العربة!",
    "product.contactSeller": "تواصل مع البائع",
    "product.addedToWishlist": "تمت الإضافة إلى المفضلة!",
    "product.removeFromWishlist": "إزالة من المفضلة",
    "product.removedFromWishlist": "تمت الإزالة من المفضلة.",
    "product.draftRestored": "تمت استعادة المسودة",
    "product.discardDraft": "تجاهل المسودة",
    "product.invalidImageType": "يسمح فقط بصور JPG وPNG وWebP.",
    "product.imageTooLarge": "يجب أن يكون حجم الصورة أقل من 5 ميجابايت.",
    "product.wishlistUpdated": "تم تحديث المفضلة!",
    "wishlist.confirmRemove": "إزالة من المفضلة؟",
    "wishlist.confirmRemoveDesc": "سيتم إزالة هذا العنصر من قائمة رغباتك.",
    "product.statusAvailable": "متوفر",
    "product.statusSold": "تم البيع",
    "product.statusDraft": "مسودة",
    "product.statusRejected": "مرفوض",
    "product.statusSuspended": "موقوف",
    "product.statusPendingReview": "قيد المراجعة",
    "product.rating": "التقييم",
    "product.comment": "التعليق",
    "product.productId": "معرف المنتج",

    // Auctions
    "auctions.title": "المزادات",
    "auctions.metaTitle": "مزادات الأسماك الحية",
    "auctions.metaDesc":
      "انضم إلى مزادات الأسماك الحية على صياد. مزايدة على الصيد الطازج.",
    "auctions.search": "ابحث عن مزادات...",
    "auctions.allStatus": "جميع الحالات",
    "auctions.active": "نشط",
    "auctions.finished": "منتهي",
    "auctions.cancelled": "ملغي",
    "auctions.loadError": "خطأ في تحميل المزادات",
    "auctions.noAuctionsDesc":
      "لا توجد مزادات تطابق التصفية الحالية. تصفح المنتجات أو تحقق من المزادات المنتهية.",

    // Auction Detail
    "auction.currentBid": "المزايدة الحالية",
    "auction.startingPrice": "سعر البداية",
    "auction.reservePrice": "سعر الاحتياطي",
    "auction.bidIncrement": "الزيادة الدنيا",
    "auction.totalBids": "إجمالي المزايدات",
    "auction.start": "البداية",
    "auction.end": "النهاية",
    "auction.winner": "الفائز",
    "auction.placeBid": "تقديم مزايدة",
    "auction.placingBid": "جارٍ التقديم...",
    "auction.bidHistory": "سجل المزايدات",
    "auction.noBids": "لا توجد مزايدات بعد. كن أول من يزايد!",
    "auction.bidPlaced": "تم تقديم المزايدة بنجاح!",
    "auction.invalidBid": "الرجاء إدخال مبلغ مزايدة صحيح.",
    "auction.ended": "انتهى",
    "auction.outbid": "تم تجاوز مزايدتك!",
    "auction.endingSoon": "سينتهي قريباً!",
    "auction.newBid": "تم تقديم مزايدة جديدة!",
    "auction.startAuction": "بدء مزاد",
    "auction.startNew": "بدء مزاد جديد",
    "auction.bidCustomerOnly": "يمكن للعملاء فقط تقديم عروض.",
    "auction.loginToBid": "سجل الدخول كعميل لتقديم عروض.",
    "auction.autoBid": "مزايدة تلقائية",
    "auction.autoBidMaxRequired": "أدخل حد مزايدة تلقائية أعلى من مزايدتك.",
    "auction.reserveConfirmationPending": "لم يصل المزاد إلى سعر الاحتياطي. يجب على البائع قبول أو رفض أعلى مزايدة.",
    "auction.confirmBefore": "قم بالرد قبل",
    "auction.acceptBid": "قبول المزايدة",
    "auction.rejectBid": "رفض المزايدة",
    "auction.acceptReserveTitle": "قبول أعلى مزايدة؟",
    "auction.acceptReserveConfirm": "اقبل هذه المزايدة وأنهِ تسوية المزاد.",
    "auction.rejectReserveTitle": "رفض أعلى مزايدة؟",
    "auction.rejectReserveConfirm": "ارفض هذه المزايدة، وألغِ المزاد، وحرر الأموال المحجوزة.",
    "auction.reserveAccepted": "تم قبول المزايدة وتسوية المزاد.",
    "auction.reserveRejected": "تم رفض المزايدة وإلغاء المزاد.",
    "auction.statusActive": "نشط",
    "auction.statusFinished": "منتهي",
    "auction.statusCancelled": "ملغي",
    "auction.statusScheduled": "مجدول",
    "auction.statusPendingSellerConfirmation": "بانتظار تأكيد البائع",
    "auction.statusConfirmationExpired": "انتهت مهلة التأكيد",

    // Cart
    "cart.title": "عربة التسوق",
    "cart.clear": "تفريغ العربة",
    "cart.product": "المنتج",
    "cart.price": "السعر",
    "cart.quantity": "الكمية",
    "cart.subtotal": "المجموع الفرعي",
    "cart.total": "الإجمالي",
    "cart.checkout": "متابعة الشراء",
    "checkout.title": "الدفع",
    "cart.empty": "عربة التسوق فارغة",
    "cart.removeItemConfirm": "هل أنت متأكد أنك تريد إزالة هذا العنصر؟",
    "cart.emptyDesc": "تصفح المنتجات لإضافة عناصر إلى عربة التسوق.",
    "cart.browseProducts": "تصفح المنتجات",
    "cart.cleared": "تم تفريغ العربة!",
    "cart.itemRemoved": "تم إزالة العنصر من العربة.",
    "cart.placeOrder": "تقديم الطلب",
    "cart.insufficientWallet":
      "رصيد المحفظة التجريبي غير كافٍ. يرجى إضافة رصيد تجريبي أولاً.",
    "cart.placingOrder": "جاري تقديم الطلب...",
    "cart.shippingAddress": "عنوان الشحن",
    "cart.city": "المدينة",
    "cart.addressLine": "العنوان",
    "cart.paymentMethod": "طريقة الدفع",
    "cart.demoPaymentNotice":
      "الدفع تجريبي فقط: يستخدم هذا الطلب رصيد محفظة تجريبي. لن يتم خصم أي أموال حقيقية.",
    "cart.creditCard": "بطاقة ائتمان تجريبية",
    "cart.cashOnDelivery": "دفع عند الاستلام تجريبي",
    "cart.requiredFields": "الاسم الكامل ورقم الهاتف والعنوان والمدينة مطلوبة.",
    "cart.orderSuccess": "تم تقديم الطلب بنجاح!",
    "cart.orderSuccessDesc": "تم تقديم طلبك. جاري التحويل إلى لوحة التحكم...",
    "cart.backToCart": "العودة إلى العربة",
    "cart.removeItemTitle": "إزالة العنصر",
    "cart.clearConfirm": "هل أنت متأكد من رغبتك في إفراغ سلة التسوق بالكامل؟",
    "cart.orderError": "فشل تقديم الطلب",
    "cart.insufficientStock": "متوفر {stock} فقط",
    "cart.maxReached": "الحد الأقصى",

    // Dashboard
    "dash.title": "لوحة التحكم",
    "dash.tabsLabel": "تبويبات لوحة التحكم",
    "dash.overview": "نظرة عامة",
    "dash.orders": "الطلبات",
    "dash.products": "المنتجات",
    "dash.productsNotAvailable": "غير متاح لدورك",
    "dash.auctions": "المزادات",
    "dash.wishlist": "المفضلة",
    "dash.sellerDashboard": "لوحة البائع",
    "dash.addresses": "العناوين",
    "dash.notifications": "الإشعارات",
    "dash.profile": "الملف الشخصي",
    "dash.changePassword": "تغيير كلمة المرور",
    "dash.welcome": "مرحباً بعودتك",
    "dash.role": "الدور",
    "dash.totalOrders": "إجمالي الطلبات المقدمة",
    "dash.yourProducts": "منتجاتك المدرجة",
    "dash.pendingReviews": "المراجعات المعلقة",
    "dash.productsAwaitingReview": "منتجات بانتظار المراجعة",
    "dash.viewAdminPanel": "→ لوحة الإدارة",
    "dash.totalUsers": "إجمالي المستخدمين",
    "dash.registeredUsers": "مستخدم مسجل",
    "dash.noOrders": "لا توجد طلبات بعد.",
    "dash.noProducts": "لم تقم بإدراج أي منتجات بعد.",
    "dash.emptyWishlist": "المفضلة فارغة.",
    "dash.noNotifications": "لا توجد إشعارات.",
    "dash.updateProfile": "تحديث الملف الشخصي",
    "dash.updating": "جارٍ التحديث...",
    "dash.profileUpdated": "تم تحديث الملف الشخصي بنجاح!",
    "dash.currentPassword": "كلمة المرور الحالية",
    "dash.newPassword": "كلمة المرور الجديدة",
    "dash.changePwBtn": "تغيير كلمة المرور",
    "dash.changing": "جارٍ التغيير...",
    "dash.passwordChanged": "تم تغيير كلمة المرور بنجاح!",
    "dash.orderNum": "طلب #",
    "dash.date": "التاريخ",
    "dash.view": "عرض",
    "dash.myProducts": "منتجاتي",
    "profile.title": "ملفي الشخصي",
    "profile.uploadPhoto": "انقر لرفع صورة",
    "profile.imageTooLarge": "يجب أن تكون الصورة أقل من 5 ميجابايت.",
    "profile.invalidImageType": "اختر صورة بصيغة JPG أو PNG أو WebP.",
    "profile.invalidImageContent": "الملف المحدد ليس صورة صالحة بصيغة مدعومة.",
    "profile.uploadNoUrl": "الرفع لم يُرجع رابطًا.",
    "profile.photoUpdated": "تم تحديث الصورة الشخصية!",
    "profile.photoRemoved": "تم إزالة الصورة الشخصية.",
    "profile.confirmDeleteTitle": "حذف الصورة",
    "profile.confirmDeletePhoto": "هل أنت متأكد من إزالة الصورة الشخصية؟",

    // Verify Email
    "verify.title": "تأكيد البريد الإلكتروني",
    "verify.success":
      "تم تأكيد البريد الإلكتروني بنجاح! يمكنك تسجيل الدخول الآن.",
    "verify.error": "فشل التحقق. الرابط قد يكون منتهياً أو غير صالح.",
    "verify.loggingIn": "جارٍ تسجيل الدخول تلقائياً...",
    "verify.redirecting": "جاري التحويل إلى تسجيل الدخول...",
    "verify.invalidLink": "رابط تحقق غير صالح.",
    "verify.waitingTitle": "تحقق من بريدك الإلكتروني",
    "verify.waitingDesc": "لقد أرسلنا رابط التحقق إلى",
    "verify.alreadyVerified": "لقد قمت بالتحقق بالفعل",
    "verify.useOtherEmail": "استخدام بريد إلكتروني آخر",
    "verify.successTitle": "تم التحقق!",
    "verify.successDesc": "جارٍ تسجيل الدخول...",
    "verify.resend": "إعادة إرسال بريد التحقق",
    "verify.resending": "جارٍ الإرسال...",
    "verify.resendSuccess": "تم إرسال بريد التحقق! تحقق من بريدك الوارد.",
    "verify.resendError": "تعذر إعادة إرسال بريد التحقق. حاول مرة أخرى.",

    // Forgot password token entry
    "auth.tokenPlaceholder": "الصق الرمز أو الرابط من بريدك الإلكتروني",
    "auth.tokenHint":
      "الصق رابط البريد الإلكتروني بالكامل — سنستخرج الرمز تلقائياً",

    // Shipping Addresses
    "shipping.title": "عناوين الشحن",
    "shipping.addNew": "إضافة عنوان جديد",
    "shipping.fullName": "الاسم الكامل",
    "shipping.phone": "رقم الهاتف",
    "shipping.city": "المدينة",
    "shipping.governorate": "المحافظة",
    "shipping.addressLine": "العنوان",
    "shipping.postalCode": "الرمز البريدي",
    "shipping.save": "حفظ العنوان",
    "shipping.saving": "جارٍ الحفظ...",
    "shipping.delete": "حذف",
    "shipping.noAddresses": "لا توجد عناوين شحن بعد.",
    "shipping.savedAddresses": "العناوين المحفوظة",
    "shipping.saved": "تم حفظ العنوان!",
    "shipping.deleted": "تم حذف العنوان!",
    "shipping.confirmDelete": "حذف هذا العنوان؟",
    "shipping.confirmDeleteDesc": "سيتم حذف هذا العنوان نهائياً.",
    "shipping.error": "فشل حفظ العنوان.",

    // Seller Profile
    "auctionDetail.title": "تفاصيل المزاد",
    "privacy.title": "سياسة الخصوصية",
    "terms.title": "الشروط والأحكام",

    // Seller
    "seller.title": "الملف الشخصي للبائع",
    "seller.myProfile": "ملفي الشخصي كبائع",
    "seller.storeName": "اسم المتجر",
    "seller.description": "الوصف",
    "seller.contactEmail": "البريد الإلكتروني للتواصل",
    "seller.contactPhone": "رقم الهاتف للتواصل",
    "seller.location": "الموقع",
    "seller.save": "حفظ الملف الشخصي",
    "seller.saving": "جارٍ الحفظ...",
    "seller.create": "إنشاء ملف بائع",
    "seller.saved": "تم حفظ الملف الشخصي!",
    "seller.notFound": "لم يتم العثور على ملف البائع.",
    "seller.rating": "التقييم",
    "seller.totalSales": "إجمالي المبيعات",
    "seller.totalProducts": "إجمالي المنتجات",
    "seller.recentOrders": "الطلبات الأخيرة",
    "seller.noProfile": "لم تقم بإنشاء ملف بائع بعد.",
    "seller.becomeSeller": "كن بائعاً",
    "seller.memberSince": "عضو منذ",
    "seller.listings": "المنتجات المعروضة",
    "seller.setupRequired": "قم بإعداد ملف البائع",
    "seller.setupDesc": "أكمل ملف البائع قبل إدراج المنتجات.",

    // Product Management
    "product.create": "إضافة منتج",
    "product.edit": "تعديل المنتج",
    "product.title": "العنوان",
    "product.description": "الوصف",
    "product.price": "السعر",
    "product.new": "جديد",
    "product.used": "مستعمل",
    "product.conditionNew": "جديد",
    "product.conditionUsed": "مستعمل",
    "product.images": "الصور",
    "product.uploadImage": "رفع صورة",
    "product.uploading": "جارٍ الرفع...",
    "product.save": "حفظ المنتج",
    "product.saving": "جارٍ الحفظ...",
    "product.saved": "تم حفظ المنتج!",
    "product.deleted": "تم حذف المنتج.",
    "product.deleteConfirm": "حذف هذا المنتج؟ لا يمكن التراجع عن هذا الإجراء.",
    "product.deleteImage": "إزالة",
    "product.chooseImage": "اختر صورة",
    "product.imageError": "فشل رفع الصورة.",
    "product.addAnother": "إضافة أخرى",

    // Admin
    "admin.title": "لوحة الإدارة",
    "admin.users": "المستخدمين",
    "admin.reports": "البلاغات",
    "admin.products": "المنتجات",
    "admin.orders": "الطلبات",
    "admin.categories": "التصنيفات",
    "admin.plans": "الخطط",
    "admin.id": "المعرف",
    "admin.name": "الاسم",
    "admin.email": "البريد الإلكتروني",
    "admin.role": "الدور",
    "admin.status": "الحالة",
    "admin.active": "نشط",
    "admin.inactive": "غير نشط",
    "admin.suspended": "موقوف",
    "admin.suspend": "إيقاف",
    "admin.activate": "تفعيل",
    "admin.toggleStatus": "تغيير الحالة",
    "admin.reportReason": "السبب",
    "admin.reportStatus": "الحالة",
    "admin.resolve": "حل",
    "admin.dismiss": "رفض",
    "admin.addCategory": "إضافة تصنيف",
    "admin.categoryName": "اسم التصنيف",
    "admin.categoryDesc": "الوصف",
    "admin.noAccess": "ليس لديك صلاحية الوصول للإدارة.",
    "admin.userToggled": "تم تحديث حالة المستخدم!",
    "admin.confirmDeleteCategory": "حذف التصنيف؟",
    "admin.confirmDeleteCategoryDesc": "قد تتأثر جميع المنتجات في هذه الفئة.",
    "admin.categoryDeleted": "تم حذف التصنيف",
    "admin.categoryAdded": "تم إضافة التصنيف",
    "admin.reportResolved": "تم حل البلاغ",
    "admin.reportType": "النوع",
    "admin.reportTarget": "الهدف",
    "admin.reportMessage": "الرسالة",
    "admin.reportAdminNote": "ملاحظة المشرف",
    "admin.resolvedAt": "تم الحل في",
    "admin.pendingFreezes": "التجميد المعلق",
    "admin.pendingReports": "البلاغات المعلقة",
    "admin.dismissReport": "رفض البلاغ",
    "admin.dismissReportConfirm": "رفض هذا البلاغ بدون إجراء؟",
    "admin.reportDismissed": "تم رفض البلاغ.",
    "admin.productStatusUpdated": "تم تحديث حالة المنتج",
    "admin.planCreated": "تم إنشاء الخطة",
    "admin.planUpdated": "تم تحديث الخطة",
    "admin.planDeleted": "تم حذف الخطة",
    "admin.confirmDeletePlan": "حذف الخطة؟",
    "admin.confirmDeletePlanDesc": "لا يمكن التراجع عن هذا.",
    "admin.revenue": "الإيرادات",
    "admin.review": "مراجعة",
    "admin.approve": "موافقة",
    "admin.reject": "رفض",
    "admin.rejectionReason": "سبب الرفض",
    "admin.confirmApprove": "الموافقة على هذا المنتج؟",
    "admin.confirmApproveDesc": "سيصبح المنتج مرئياً لجميع المستخدمين.",
    "admin.productApproved": "تمت الموافقة على المنتج!",
    "admin.productRejected": "تم رفض المنتج",
    "admin.noPendingReviews": "لا توجد منتجات بانتظار المراجعة",
    "admin.productsAwaiting": "منتجات تنتظر مراجعتك",
    "admin.platformBalance": "رصيد المنصة",
    "admin.totalFees": "إجمالي الرسوم المحصلة",
    "admin.feeIncome": "دخل الرسوم",
    "admin.addPlan": "إضافة خطة",
    "admin.editPlan": "تعديل الخطة",
    "admin.addSubscriptionPlan": "إضافة خطة اشتراك",
    "admin.planDescription": "الوصف",
    "admin.planPrice": "السعر",
    "admin.planPriceEgp": "السعر (جنيه)",
    "admin.planAuctions": "المزادات",
    "admin.planBids": "المزايدات",
    "admin.planRequests": "الطلبات",
    "admin.planMaxAuctions": "الحد الأقصى للمزادات/شهر",
    "admin.planMaxBids": "الحد الأقصى للمزايدات/شهر",
    "admin.planMaxRequests": "الحد الأقصى للطلبات/شهر",
    "admin.planSortOrder": "ترتيب العرض",
    "admin.confirmSuspendUser": "إيقاف المستخدم؟",
    "admin.confirmSuspendUserDesc": "إيقاف {user}؟ سيفقد الوصول حتى يتم تفعيله مرة أخرى.",
    "admin.confirmActivateUser": "تفعيل المستخدم؟",
    "admin.confirmActivateUserDesc": "تفعيل {user} واستعادة الوصول للحساب.",
    "admin.confirmApproveRole": "الموافقة على طلب الدور؟",
    "admin.confirmApproveRoleDesc": "الموافقة على دور {role} للمستخدم {user}؟",
    "admin.confirmRejectRole": "رفض طلب الدور؟",
    "admin.confirmRejectRoleDesc": "رفض طلب دور {role} للمستخدم {user}؟",

    // Order Detail
    "order.title": "تفاصيل الطلب",
    "order.items": "المنتجات",
    "order.total": "الإجمالي",
    "order.status": "الحالة",
    "order.statusPending": "قيد الانتظار",
    "order.statusPaid": "تم الدفع",
    "order.statusShipped": "تم الشحن",
    "order.statusDelivered": "تم التوصيل",
    "order.statusCancelled": "ملغي",
    "order.date": "التاريخ",
    "order.buyer": "المشتري",
    "order.seller": "البائع",
    "order.summary": "ملخص الطلب",
    "order.shipping": "الشحن",
    "order.quantity": "الكمية",
    "order.price": "السعر",
    "order.subtotal": "المجموع الفرعي",
    "order.backToOrders": "العودة للطلبات",
    "order.notFound": "الطلب غير موجود.",
    "order.cancel": "إلغاء الطلب",
    "order.cancelling": "جاري الإلغاء...",
    "order.cancelled": "تم إلغاء الطلب بنجاح!",
    "order.cancelConfirm":
      "هل أنت متأكد من إلغاء هذا الطلب؟ لا يمكن التراجع عن هذا الإجراء.",
    "order.cancelError": "تعذر إلغاء الطلب.",
    "order.statusReturnRequested": "طلب إرجاع",
    "order.statusReturned": "تم الإرجاع",
    "order.deliveredAt": "تاريخ التوصيل",
    "order.returnReason": "سبب الإرجاع",
    "order.requestReturn": "طلب إرجاع",
    "order.returnConfirm": "هل أنت متأكد من طلب الإرجاع؟ سيتم إخطار البائع.",
    "order.returnPending": "طلب الإرجاع قيد الانتظار. في انتظار موافقة المشرف.",
    "order.returnRequested": "تم تقديم طلب الإرجاع بنجاح!",
    "order.returnError": "تعذر معالجة طلب الإرجاع.",
    "order.approveReturn": "الموافقة على الإرجاع",
    "order.approveReturnConfirm": "الموافقة على طلب الإرجاع؟ سيتم استرداد أموال المشتري.",
    "order.returnApproved": "تمت الموافقة على الإرجاع واسترداد الأموال.",
    "order.rejectReturn": "رفض الإرجاع",
    "order.rejectReturnReason": "سبب رفض الإرجاع:",
    "order.rejectReturnConfirm": "رفض طلب الإرجاع؟",
    "order.returnRejected": "تم رفض طلب الإرجاع.",
    // Reviews
    "review.title": "التقييمات",
    "review.writeReview": "اكتب تقييماً",
    "review.rating": "التقييم",
    "review.comment": "التعليق",
    "review.submit": "إرسال التقييم",
    "review.submitting": "جارٍ الإرسال...",
    "review.submitted": "تم إرسال التقييم!",
    "review.noReviews": "لا توجد تقييمات بعد.",
    "review.delete": "حذف",
    "review.rateProduct": "قيم هذا المنتج",
    "review.stars": "نجوم",
    "review.sort": "ترتيب التقييمات",
    "review.highestRated": "الأعلى تقييماً",
    "review.lowestRated": "الأقل تقييماً",

    // Notifications
    "notif.title": "الإشعارات",
    "notif.markRead": "تحديد كمقروء",
    "notif.markAllRead": "تحديد الكل كمقروء",
    "notif.markedAllRead": "تم تحديد الكل كمقروء",
    "notif.noNotifications": "لا توجد إشعارات",
    "notif.unreadCount": "غير مقروء",

    // Common
    "common.loading": "جارٍ التحميل...",
    "common.pageNotFound": "الصفحة غير موجودة",
    "common.pageNotFoundDesc": "الصفحة التي تبحث عنها غير موجودة.",
    "common.goHome": "العودة للرئيسية",
    "common.refresh": "تحديث",
    "common.offline": "أنت غير متصل بالإنترنت. بعض الميزات قد لا تكون متاحة.",
    "common.backOnline": "تمت استعادة الاتصال!",
    "common.somethingWentWrong": "حدث خطأ غير متوقع",
    "common.errorFallbackDesc": "حدث خطأ غير متوقع. يرجى تحديث الصفحة.",
    "common.error": "خطأ",
    "common.endsIn": "متبقي",
    "common.bids": "مزايدة",
    "common.start": "البداية",
    "products.showingCount": "عرض {count} من أصل {total} منتج",
    "common.next": "التالي",
    "common.previous": "السابق",
    "common.N/A": "غير متاح",
    "common.search": "بحث",
    "common.select": "اختيار",
    "common.required": "مطلوب",
    "common.optional": "اختياري",
    "common.linkCopied": "تم نسخ الرابط!",
    "common.refreshing": "جارٍ التحديث...",
    "common.pullToRefresh": "اسحب للتحديث",
    "common.confirm": "تأكيد",
    "common.hot": "رائج",
    "common.qty": "الكمية",
    "common.retry": "إعادة المحاولة",
    "common.days": "أيام",
    "common.hours": "ساعات",
    "common.minutes": "دقائق",
    "common.seconds": "ثوان",
    "common.page": "صفحة",
    "common.clearFilters": "مسح التصفية",
    "common.loadFailed": "فشل التحميل",
    "common.remove": "إزالة",
    "common.delete": "حذف",
    "common.save": "حفظ",
    "common.recent": "الأخيرة",
    "common.recentlyViewed": "تم العرض مؤخراً",
    "common.quickLinks": "روابط سريعة",
    "common.cancel": "إلغاء",
    "common.close": "إغلاق",
    "common.dismiss": "تجاهل",
    "common.breadcrumb": "مسار التنقل",
    "common.product": "منتج",
    "common.category": "فئة",
    "common.amountPlaceholder": "0.00",
    "common.back": "رجوع",
    "common.title": "العنوان",
    "common.name": "الاسم",
    "common.tier": "المستوى",
    "common.status": "الحالة",
    "common.actions": "الإجراءات",
    "common.view": "عرض",
    "common.free": "مجاني",
    "common.secureCheckout": "دفع آمن",
    "common.easyReturns": "إرجاع سهل",
    "auth.loginRequired": "يرجى تسجيل الدخول للمتابعة.",

    // Auction Requests
    "auctionRequests.title": "طلبات المزادات",
    "auctionRequests.submit": "تقديم طلب مزاد",
    "auctionRequests.myRequests": "طلباتي",
    "auctionRequests.productTitle": "اسم المنتج",
    "auctionRequests.productDescription": "الوصف",
    "auctionRequests.estimatedValue": "القيمة المقدرة (EGP)",
    "auctionRequests.quantityKg": "الكمية (كجم)",
    "auctionRequests.fishType": "نوع السمك",
    "auctionRequests.catchLocation": "مكان الصيد",
    "auctionRequests.catchDate": "تاريخ الصيد",
    "auctionRequests.submitting": "جاري التقديم...",
    "auctionRequests.submitted": "تم تقديم الطلب بنجاح!",
    "auctionRequests.status": "الحالة",
    "auctionRequests.createdAt": "تاريخ التقديم",
    "auctionRequests.rejectionReason": "السبب",
    "auctionRequests.pending": "قيد الانتظار",
    "auctionRequests.approved": "تمت الموافقة",
    "auctionRequests.rejected": "مرفوض",
    "auctionRequests.noRequests": "لا توجد طلبات مزادات بعد",
    "auctionRequests.noRequestsDesc":
      "قدم طلباً لكي يقوم المزاد بعرض محصولك في مزاد.",
    "auctionRequests.requestAuction": "طلب مزاد",
    "auctionRequests.imageUrl": "رابط صورة المنتج",
    "auctionRequests.imageUrlHelp": "اختياري — يمكنك إضافة رابط لصورة المنتج",

    // Auction Requests Review
    "auctionRequestsReview.title": "مراجعة طلبات المزادات",
    "auctionRequestsReview.pending": "قيد الانتظار",
    "auctionRequestsReview.approved": "تمت الموافقة",
    "auctionRequestsReview.rejected": "مرفوض",
    "auctionRequestsReview.all": "الكل",
    "auctionRequestsReview.approve": "موافقة",
    "auctionRequestsReview.reject": "رفض",
    "auctionRequestsReview.approving": "جاري الموافقة...",
    "auctionRequestsReview.rejecting": "جاري الرفض...",
    "auctionRequestsReview.approvedSuccess": "تمت الموافقة على الطلب بنجاح!",
    "auctionRequestsReview.rejectedSuccess": "تم رفض الطلب بنجاح!",
    "auctionRequestsReview.rejectionReason": "سبب الرفض",
    "auctionRequestsReview.rejectionReasonPlaceholder":
      "اشرح سبب رفض هذا الطلب",
    "auctionRequestsReview.noPending": "لا توجد طلبات مزادات معلقة",
    "auctionRequestsReview.noPendingDesc": "تم مراجعة جميع الطلبات.",
    "auctionRequestsReview.confirmApprove":
      "هل أنت متأكد من الموافقة على هذا الطلب؟",
    "auctionRequestsReview.confirmReject": "هل أنت متأكد من رفض هذا الطلب؟",
    "auctionRequestsReview.fisherman": "الصياد",
    "auctionRequestsReview.actions": "الإجراءات",

    // Auctioneer Analytics
    "analytics.statusDistribution": "توزيع المزادات",
    "analytics.title": "تحليلات المزادات",
    "analytics.totalAuctions": "إجمالي المزادات",
    "analytics.activeAuctions": "المزادات النشطة",
    "analytics.finishedAuctions": "المزادات المنتهية",
    "analytics.totalBids": "إجمالي المزايدات",
    "analytics.totalRevenue": "إجمالي الإيرادات",
    "analytics.totalFees": "إجمالي الرسوم المكتسبة",
    "analytics.feeIncome": "دخل الرسوم",
    "analytics.availableInWallet": "في المحفظة",
    "analytics.recentAuctions": "أحدث المزادات",
    "analytics.startingPrice": "سعر البداية",
    "analytics.currentPrice": "السعر الحالي",
    "analytics.bidCount": "العروض",
    "analytics.endTime": "وقت الانتهاء",
    "analytics.noData": "لا توجد بيانات مزادات متاحة",

    // Scheduling
    "scheduling.startTime": "وقت بدء المزاد",
    "scheduling.endTime": "وقت انتهاء المزاد",
    "scheduling.startTimeRequired": "يرجى اختيار وقت بدء المزاد",
    "scheduling.scheduleAuction": "جدولة المزاد",
    "scheduling.scheduling": "جاري الجدولة...",
    "scheduling.info":
      "حدد موعد بدء المزاد واختيارياً موعد انتهائه. الوقت الافتراضي للانتهاء هو 7 أيام من البداية.",

    // Wallet
    "wallet.title": "محفظتي",
    "wallet.pageLabel": "المحفظة",
    "wallet.actions": "إجراءات المحفظة",
    "wallet.loadingTransactions": "جارٍ تحميل المعاملات",
    "wallet.closeAction": "إغلاق نافذة إجراء المحفظة",
    "wallet.transactionHistory": "سجل المعاملات",
    "wallet.minimumDeposit": "الحد الأدنى 10 جنيه",
    "wallet.balance": "الرصيد الإجمالي",
    "wallet.held": "الرصيد المحتجز",
    "wallet.available": "الرصيد التجريبي المتاح",
    "wallet.deposit": "إضافة رصيد تجريبي",
    "wallet.withdraw": "سحب الرصيد",
    "wallet.enterAmount": "أدخل المبلغ",
    "wallet.invalidAmount": "الرجاء إدخال مبلغ صحيح موجب",
    "wallet.amountTooLarge": "الحد الأقصى للإيداع هو 100,000 جنيه",
    "wallet.invalidDecimal": "يسمح بمنزلتين عشريتين فقط",
    "wallet.depositSuccess": "تمت إضافة الرصيد التجريبي بنجاح!",
    "wallet.depositError": "تعذرت إضافة الرصيد التجريبي. حاول مرة أخرى.",
    "wallet.transactions": "المعاملات",
    "wallet.date": "التاريخ",
    "wallet.type": "النوع",
    "wallet.description": "الوصف",
    "wallet.amount": "المبلغ",
    "wallet.noTransactions": "لا توجد معاملات بعد",
    "wallet.loadError": "فشل تحميل بيانات المحفظة.",
    "wallet.metaDesc": "إدارة رصيد وسجل معاملات محفظتك في صياد.",
    "wallet.minAmountError": "الرجاء إدخال مبلغ صحيح (الحد الأدنى 10 جنيه)",
    "wallet.maxAmountError": "الحد الأقصى للشحن 50,000 جنيه",
    "wallet.topUpSuccess": "تمت إضافة الرصيد التجريبي بنجاح!",
    "wallet.confirmTopUp": "إضافة رصيد تجريبي",
    "wallet.platformFee": "رسوم المنصة",
    "wallet.subPayment": "دفعة اشتراك تجريبية",
    "wallet.auctionPayment": "دفعة مزاد تجريبية",
    "wallet.auctionPayout": "دفعة بائع تجريبية",
    "wallet.readOnly": "المحفظة في وضع القراءة فقط",
    "wallet.paymentNote": "أضف رصيداً تجريبياً فوراً للاختبار. هذا لا يمثل أموالاً حقيقية.",
    "wallet.topUpTitle": "إضافة رصيد تجريبي",
    "wallet.topUpHelp":
      "سيتم إضافة رصيد اختبار فوراً. لا يتم تنفيذ تحويل أموال حقيقي أو سحب بنكي.",
    "wallet.amountLabel": "المبلغ (جنيه)",
    "wallet.withdrawTitle": "سحب الرصيد",
    "wallet.withdrawHelp": "أدخل المبلغ الذي تريد سحبه من الرصيد المتاح في محفظتك.",
    "wallet.withdrawPlaceholder": "أدخل مبلغاً موجباً",
    "wallet.withdrawAmountError": "يجب أن يكون مبلغ السحب أكبر من صفر.",
    "wallet.withdrawSuccess": "تم سحب الرصيد بنجاح!",
    "wallet.confirmWithdraw": "سحب الرصيد",
    "wallet.noTransactionsDesc": "سيظهر سجل معاملاتك هنا.",
    "wallet.colStatus": "الحالة",

    // Subscriptions
    "subscriptions.title": "خطط الاشتراك",
    "subscriptions.currentPlan": "خطتي الحالية",
    "subscriptions.noPlan": "لا توجد خطة نشطة",
    "subscriptions.free": "مجاني",
    "subscriptions.basic": "أساسي",
    "subscriptions.freeDesc": "وصول أساسي لتصفح واستكشاف السوق",
    "subscriptions.premium": "مميز",
    "subscriptions.premiumDesc": "وصول كامل مع دعم ذي أولوية وميزات إضافية",
    "subscriptions.pro": "احترافي",
    "subscriptions.proLabel": "برو",
    "subscriptions.proDesc":
      "كل ما في المميز بالإضافة إلى مزادات غير محدودة وتحليلات",
    "subscriptions.enterprise": "المؤسسية",
    "subscriptions.monthly": "شهري",
    "subscriptions.yearly": "سنوي",
    "subscriptions.perMonth": "/شهر",
    "subscriptions.perYear": "/سنة",
    "subscriptions.daysLeft": "متبقي {count} يوم",
    "subscriptions.endsToday": "ينتهي اليوم",
    "subscriptions.expired": "منتهي",
    "subscriptions.popular": "الأكثر طلباً",
    "subscriptions.feature1": "تصفح المنتجات والمزادات",
    "subscriptions.feature2": "تقديم عروض وشراء",
    "subscriptions.feature3": "إنشاء ملف بائع",
    "subscriptions.feature4": "دعم عملاء ذو أولوية",
    "subscriptions.feature5": "لوحة تحليلات متقدمة",
    "subscriptions.feature6": "طلبات مزادات غير محدودة",
    "subscriptions.feature7": "إعلانات مميزة",
    "subscriptions.upgrade": "الترقية برصيد تجريبي",
    "subscriptions.current": "الحالية",
    "subscriptions.learnMore": "اعرف المزيد",
    "subscriptions.upgradeSuccess": "تمت ترقية الاشتراك التجريبي بنجاح!",
    "subscriptions.upgradeError": "تعذر ترقية الاشتراك.",
    "subscriptions.invalidTier": "لا يمكن ترقية هذه الخطة حالياً.",
    "subscriptions.loadError": "تعذر تحميل بيانات الاشتراك.",
    "subscriptions.customerHeading": "افتح المزيد من العروض",
    "subscriptions.customerDesc":
      "اشترك للمزايدة على المزيد من المزادات والحصول على صفقات أفضل",
    "subscriptions.auctioneerHeading": "طور أعمالك",
    "subscriptions.auctioneerDesc":
      "اشترك لقبول وإدارة المزيد من المزادات بميزات متقدمة",
    "subscriptions.sellerHeading": "طور تجربتك",
    "subscriptions.sellerDesc":
      "اشترك لفتح المزيد من الإعلانات والمزادات والميزات المميزة",
    "subscriptions.insufficientFunds": "رصيد غير كافٍ",
    "subscriptions.demoPaymentNote":
      "ترقيات الاشتراك تستخدم رصيد المحفظة التجريبي. لا تتم معالجة مرجع دفع حقيقي أو تحويل أموال.",

    // Footer
    "footer.tagline": "سوق الصيد الأول في مصر — اشترِ، بع، وشارك في المزادات.",
    "footer.marketplace": "السوق",
    "footer.account": "الحساب",
    "footer.legal": "قانوني",
    "footer.sellOnSayiad": "بيع على صياد",
    "footer.myAddresses": "عناويني",
    "footer.allRights": "جميع الحقوق محفوظة.",
    "footer.madeWith": "صنع للصيادين 🎣",
    "footer.copyright": "\u00a9 2026 صياد. جميع الحقوق محفوظة.",

    // Admin
    "admin.activeAuctions": "المزادات النشطة",
    "admin.createFirstCategory": "أنشئ فئتك الأولى لتنظيم المنتجات",
    "admin.noCategories": "لم يتم العثور على فئات",
    "admin.noFees": "لا توجد معاملات رسوم بعد",
    "admin.noOrders": "لم يتم العثور على طلبات",
    "admin.noReports": "لم يتم العثور على تقارير",
    "admin.noUsers": "لم يتم العثور على مستخدمين",
    "admin.totalRevenue": "إجمالي الإيرادات",
    "admin.totalUsers": "إجمالي المستخدمين",
    "admin.pendingRoleRequests": "طلبات ترقية الدور المعلقة",
    "admin.roleApproved": "تمت الموافقة على طلب الدور!",
    "admin.roleRejected": "تم رفض طلب الدور",
    "admin.rejectRoleRequest": "رفض طلب الدور",
    "admin.manageUserReviews": "إدارة تقييمات المستخدمين",
    "admin.enterProductId": "أدخل معرف المنتج",
    "admin.noUserReviews": "لا توجد تقييمات لهذا المنتج",
    "admin.removeReview": "إزالة التقييم",
    "admin.confirmRemoveReview": "إزالة تقييم {userName}؟",
    "admin.reviewRemovedNotified": "تم إزالة التقييم. تم إبلاغ الكاتب.",

    // Auction
    "auction.anonymousBidder": "مزايد مجهول",
    "auction.autoBidMaxPlaceholder": "الحد الأقصى للمزايدة",
    "auction.bidIrreversible": "لا يمكن التراجع عن هذا",
    "auction.confirmBidMsg": "وضع مزايدة بقيمة",
    "auction.confirmBidTitle": "تأكيد المزايدة",
    "auction.item": "عنصر المزاد",
    "auction.idRequired": "معرف المزاد مطلوب.",
    "auction.userNumber": "# مستخدم",

    // Auctions
    "auctions.status": "الحالة",

    // Cart
    "cart.continueShopping": "مواصلة التسوق",
    "cart.quantityUpdateFailed": "تعذر تحديث الكمية. يرجى المحاولة مرة أخرى.",

    // Common
    "common.edit": "تعديل",
    "common.finish": "إنهاء",
    "common.processing": "جارٍ المعالجة\u2026",
    "common.share": "مشاركة",
    "common.showResults": "عرض النتائج",
    "common.step": "خطوة",
    "common.loadMore": "تحميل المزيد",
    "common.noDescription": "لا يوجد وصف",
    "common.viewProfile": "عرض الملف الشخصي",
    "common.kgUnit": "كجم",
    "common.unknown": "؟",

    // Notifications
    "notif.markAsRead": "تحديد كمقروء",

    // Order
    "order.confirmed": "مؤكد",
    "order.delivered": "تم التوصيل",
    "order.method": "طريقة الدفع",
    "order.paymentInfo": "معلومات الدفع",
    "order.processing": "قيد المعالجة",
    "order.reorder": "تسوق مرة أخرى",
    "order.shipped": "تم الشحن",
    "order.shippingInfo": "معلومات الشحن",
    "order.tax": "الضريبة",
    "order.track": "تفاصيل الطلب",

    // Products
    "products.category": "الفئة",
    "products.condition": "الحالة",

    // Profile
    "profile.completion": "اكتمال الملف الشخصي",

    // Shipping
    "shipping.editAddress": "تعديل العنوان",
    "shipping.locationPreview": "معاينة الموقع",
    "shipping.updated": "تم تحديث العنوان بنجاح",

    // Subscriptions
    "subscriptions.noPlans": "لم يتم العثور على خطط",
    "subscriptions.noPlansDesc": "خطط الاشتراك غير متوفرة حالياً",

    // Tour
    "tour.navDesc":
      "تنقل بسلاسة بين النظرة العامة والمنتجات وتتبع الطلبات والملف الشخصي",
    "tour.navTitle": "التنقل في لوحة التحكم",
    "tour.ordersDesc":
      "تابع معاملاتك وسجلات الشراء وتحديثات الشحن بشكل ديناميكي",
    "tour.ordersTitle": "تتبع الطلبات",
    "tour.productsDesc":
      "يمكن للصيادين والبائعين نشر أو تحديث العناصر المدرجة بسهولة هنا",
    "tour.productsTitle": "إدارة القوائم",
    "tour.welcomeToSayiad": "جاهز لاستخدام صياد! استمتع بتجربتك",

    // Pre-UI translation coverage
    "admin.remove": "إزالة",
    "auctions.bids": "مزايدات",
    "auctions.ended": "انتهى",
    "auctions.ending": "ينتهي قريباً",
    "auctions.noAuctions": "لا توجد مزادات",
    "auth.acceptTerms": "أوافق على",
    "auth.codePlaceholder": "أدخل كود التحقق",
    "auth.codeSent": "أرسلنا كود التحقق إلى بريدك الإلكتروني.",
    "auth.confirmPasswordPlaceholder": "أكد كلمة المرور",
    "auth.emailVerified": "تم تأكيد البريد الإلكتروني بنجاح.",
    "auth.enterCode": "أدخل كود التحقق",
    "auth.enterEmail": "أدخل بريدك الإلكتروني",
    "auth.enterNewPassword": "أدخل كلمة المرور الجديدة",
    "auth.error": "خطأ في المصادقة",
    "auth.haveAccount": "لديك حساب بالفعل؟",
    "auth.invalidCode": "كود التحقق غير صالح.",
    "auth.loginDesc": "سجل الدخول للمتابعة إلى حسابك في صياد.",
    "auth.invalidCredentials": "البريد الإلكتروني أو كلمة المرور غير صحيحة.",
    "auth.loginError": "تعذر تسجيل الدخول. تحقق من بياناتك وحاول مرة أخرى.",
    "auth.mustAcceptTerms": "يجب الموافقة على الشروط قبل إنشاء الحساب.",
    "auth.passwordPlaceholder": "أدخل كلمة المرور",
    "auth.redirectingToLogin": "جارٍ التحويل إلى تسجيل الدخول...",
    "auth.registerDesc": "أنشئ حسابك في صياد واختر دورك في السوق.",
    "auth.registerError": "تعذر إنشاء الحساب. يرجى المحاولة مرة أخرى.",
    "auth.resendIn": "يمكنك طلب كود آخر خلال",
    "auth.sendCode": "إرسال الكود",
    "auth.verificationFailed": "فشل تأكيد البريد الإلكتروني.",
    "auth.verifyPrompt": "يرجى تأكيد بريدك الإلكتروني قبل تسجيل الدخول.",
    "auth.welcomeBack": "مرحباً بعودتك",
    "common.action": "الإجراء",
    "common.auction": "مزاد",
    "common.date": "التاريخ",
    "common.errorOccurred": "حدث خطأ.",
    "common.hidePassword": "إخفاء كلمة المرور",
    "common.of": "من",
    "common.showPassword": "إظهار كلمة المرور",
    "common.tryAgain": "حاول مرة أخرى",
    "common.yearsOld": "سنة",
    "error_service_unavailable": "الخدمة غير متاحة",
    "error_service_unavailable_desc": "الخدمة غير متاحة مؤقتاً. يرجى المحاولة بعد قليل.",
    "footer.privacy": "سياسة الخصوصية",
    "footer.quickLinks": "روابط سريعة",
    "footer.rights": "جميع الحقوق محفوظة.",
    "footer.shipping": "الشحن",
    "footer.support": "الدعم",
    "footer.terms": "الشروط",
    "nav.profile": "الملف الشخصي",
    "profile.removePhoto": "إزالة الصورة",
    "register.auctioneer": "مزاد",
    "register.auctioneerDesc": "إدارة المزادات ومراجعة طلبات المزاد.",
    "register.baitSeller": "بائع طعم",
    "register.baitSellerDesc": "بيع الطُعم ومستلزمات الصيد.",
    "register.customer": "عميل",
    "register.customerDesc": "شراء المنتجات والمزايدة في المزادات.",
    "register.fisherman": "صياد",
    "register.fishermanDesc": "بيع الصيد وطلب إنشاء المزادات.",
    "retry": "إعادة المحاولة",
    "subscriptions.baitSellerDesc": "استخدم أدوات البيع للطُعم ومستلزمات الصيد.",
    "subscriptions.baitSellerHeading": "طوّر متجر الطُعم",
    "subscriptions.fishermanDesc": "فعّل أدوات البيع لقوائم الصيد وطلبات المزاد.",
    "subscriptions.fishermanHeading": "طوّر نشاطك في الصيد",
    "validation.invalidEmail": "يرجى إدخال بريد إلكتروني صحيح.",
    "validation.invalidPhone": "يرجى إدخال رقم هاتف صحيح.",
    "validation.passwordsNotMatch": "كلمات المرور غير متطابقة.",

    // Validation
    "validation.invalid": "قيمة غير صالحة",
    "validation.minLength": "القيمة قصيرة جداً",
    "validation.required": "هذا الحقل مطلوب",
    "validation.minAge": "يجب أن يكون عمرك {minAge} سنة على الأقل",
  },
};

import { KEYS } from '../constants/storage-keys.js';

let currentLang = localStorage.getItem(KEYS.LANG) || "en";

export function t(key, replacements = {}) {
  let message = translations[currentLang]?.[key] || translations.en[key] || key;
  for (const placeholder in replacements) {
    message = message.replace(`{${placeholder}}`, replacements[placeholder]);
  }
  return message;
}

export function pluralize(key, count, lang = currentLang) {
  const rule = new Intl.PluralRules(lang === 'ar' ? 'ar-EG' : 'en-US');
  const category = rule.select(count);
  const pluralKey = `${key}.${category}`;
  const msg = t(pluralKey);
  const finalMsg = msg === pluralKey ? t(key) : msg;
  return finalMsg.replace('{count}', count);
}

export function setLanguage(lang) {
  if (!translations[lang]) return;
  currentLang = lang;
  localStorage.setItem("sayiad_lang", lang);
  document.documentElement.lang = lang === "ar" ? "ar" : "en";
  document.documentElement.dir = lang === "ar" ? "rtl" : "ltr";
  updateStaticText();
}

export function getCurrentLang() {
  return currentLang;
}

export function updateStaticText() {
  document.querySelectorAll("[data-i18n]").forEach((el) => {
    const key = el.dataset.i18n;
    el.textContent = t(key);
  });
  document.querySelectorAll("[data-i18n-title]").forEach((el) => {
    el.title = t(el.dataset.i18nTitle);
  });
  document.querySelectorAll("[data-i18n-placeholder]").forEach((el) => {
    el.placeholder = t(el.dataset.i18nPlaceholder);
  });
}
