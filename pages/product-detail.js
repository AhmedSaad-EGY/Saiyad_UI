async function renderProductDetail(container, route, params) {
  const id = params.id;
  if (!id) { showError(container, 'Product ID is required.'); return; }
  ...
  try {
    ...
    const allImages = [p.primaryImageUrl, ...(p.images || []).map(i => i.imageUrl || i)].filter(Boolean);
        ...
        <div class="detail-info">
          ...
          <div style="margin-top:32px;padding-top:20px;border-top:1px solid var(--border)" id="reviewsSection">
            ...
            <div id="reviewsList">
              ...
              `).join('') : `<p style="color:var(--text-muted);text-align:center;padding:20px">${t('review.noReviews')}</p>`}
            </div>
          </div>
        </div>
      </div>
    ...
    trackRecentlyViewed(p.id, p.title, p.primaryImageUrl, p.price);
    ...
    $$('.thumb-img').forEach(el => {
      el.addEventListener('click', () => {
        ...
        if (main) { main.src = allImages[idx]; main.alt = ''; }
    ...
    stars.forEach(star => {
      star.addEventListener('mouseenter', () => {
        ...
        stars.forEach(s => {
          s.style.color = parseInt(s.dataset.star) <= v ? '#f59e0b' : 'var(--text-muted)';
      ...
      star.addEventListener('mouseleave', () => {
        ...
        stars.forEach(s => {
          s.style.color = parseInt(s.dataset.star) <= selected ? '#f59e0b' : 'var(--text-muted)';
      ...
      star.addEventListener('click', () => {
        ...
        stars.forEach(s => {
          s.style.color = parseInt(s.dataset.star) <= parseInt(star.dataset.star) ? '#f59e0b' : 'var(--text-muted)';
          s.style.transform = 'scale(1)';
        });
      });
    });
