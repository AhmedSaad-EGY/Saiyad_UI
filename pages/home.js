async function renderHome(container) {
  ...
  try {
    ...
    renderProductCards(document.getElementById('homeProducts'), products.items || products.data || []);
    renderAuctionCards(document.getElementById('homeAuctions'), auctions.items || auctions.data || []);
...
function renderProductCards(container, products) {
  if (!products?.length) {
    renderEmptyState(container, { icon: 'fa-box-open', title: t('home.noProducts') });
    return;
  }
  container.innerHTML = products.map((p, i) => {
    const title = p.title || 'Product';
    return `
      <a href="#/product-detail?id=${p.id}" class="product-card animate-on-scroll stagger-${Math.min(i + 1, 8)}" aria-label="${escapeHtml(title)} - ${formatPrice(p.price)}">
        <div class="product-card-img">
          ${p.primaryImageUrl ? progressiveImg(p.primaryImageUrl, title, '') : '<i class="fas fa-image" aria-hidden="true"></i>'}
          <button class="btn btn-sm btn-primary quick-view-btn" data-quickview-id="${p.id}" data-quickview-title="${escapeHtml(title)}" data-quickview-price="${p.price}" data-quickview-image="${p.primaryImageUrl || ''}" data-quickview-desc="${escapeHtml(p.description || '')}"><i class="fas fa-eye"></i> Quick View</button>
        </div>
...
function renderAuctionCards(container, auctions) {
  if (!auctions?.length) {
    renderEmptyState(container, { icon: 'fa-gavel', title: t('home.noAuctions') });
    return;
  }
