...
async function renderOverview(content, user) {
  ...
  try {
    ...
    document.getElementById('dashOrders').innerHTML = `<h3><i class="fas fa-box"></i> ${t('dash.orders')}</h3><p style="font-size:2rem;font-weight:700;color:var(--primary)">${orders.totalCount || orders.total || 0}</p><p style="color:var(--text-muted)">${t('dash.totalOrders')}</p>`;
  } catch {
    document.getElementById('dashOrders').innerHTML = `<div class="alert alert-info">${t('common.error')}</div>`;
  }
...
async function renderOrders(content) {
  ...
  async function loadOrders() {
    ...
    try {
      ...
      if (!orders.length) {
        renderEmptyState(list, { icon: 'fa-box', title: t('dash.noOrders'), actionText: t('cart.browseProducts'), actionHref: '#/products' });
        return;
      }
      ...
      `;
      document.getElementById('ordersPrev')?.addEventListener('click', () => { if (page > 1) { page--; loadOrders(); }});
      document.getElementById('ordersNext')?.addEventListener('click', () => { if (page < pages) { page++; loadOrders(); }});
      observeAnimations();
    } catch (e) {
...
async function renderMyProducts(content) {
  ...
  document.getElementById('myProductForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      if (fileInput.files.length) {
        ...
        if (upload.url) {
          await api.post(`/products/${product.id}/images`, { imageUrl: upload.url, isPrimary: true });
        }
  ...
  try {
    ...
    document.getElementById('myProductsList').innerHTML = `<div class="alert alert-error">${escapeHtml(e.message)}</div>`;
  }
}
...
async function renderWishlist(content) {
  ...
  try {
    ...
    if (!items.length) {
      renderEmptyState(document.getElementById('wishlistItems'), { icon: 'fa-heart', title: t('dash.emptyWishlist'), actionText: t('cart.browseProducts'), actionHref: '#/products' });
      return;
    }
    ...
    document.getElementById('wishlistItems').innerHTML = `<div class="alert alert-error">${escapeHtml(e.message)}</div>`;
  }
}
...
async function renderNotifications(content) {
  ...
  try {
    ...
    if (!notifs.length) {
      renderEmptyState(document.getElementById('notifList'), { icon: 'fa-bell', title: t('dash.noNotifications') });
      observeAnimations();
      return;
    }
    ...
    document.getElementById('notifList').innerHTML = `<div class="alert alert-error">${escapeHtml(e.message)}</div>`;
  }
}
...
function renderProfile(content, user) {
  ...
  document.getElementById('profileForm').addEventListener('submit', async (e) => {
    ...
    try {
      const data = await api.put('/users/profile', { fullName: name, email, phone });
...
function renderChangePassword(content) {
  ...
  document.getElementById('passwordForm').addEventListener('submit', async (e) => {
    ...
    try {
      await api.post('/auth/change-password', { currentPassword: old, newPassword: pw });
