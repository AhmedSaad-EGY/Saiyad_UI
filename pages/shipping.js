async function renderShipping(container) {
  ...
  async function loadAddresses() {
    ...
    try {
      ...
      if (!addresses.length) {
        renderEmptyState(list, { icon: 'fa-truck', title: t('shipping.noAddresses'), actionText: t('shipping.addNew'), actionFn: () => document.getElementById('showAddForm').click() });
        return;
      }
      ...
      list.querySelectorAll('.delete-addr').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          try {
            await api.del(`/shippingaddresses/${btn.dataset.id}`);
