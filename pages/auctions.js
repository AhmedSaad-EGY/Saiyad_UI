async function renderAuctions(_container, _fullPath, params) {
  ...
  async function load() {
    ...
    try {
      ...
      if (!items.length) {
        renderEmptyState(list, { icon: 'fa-gavel', title: t('home.noAuctions'), actionText: t('common.clearFilters'), actionHref: '#/auctions' });
      } else {
      ...
      for (let i = 1; i <= pages && i <= 10; i++) {
        ...
        btn.textContent = i;
        btn.onclick = () => { page = i; syncUrl(); load(); };
        pagination.appendChild(btn);
      }
    } catch (e) {
      renderEmptyState(list, { icon: 'fa-exclamation-triangle', title: t('auctions.loadError'), desc: escapeHtml(e.message), actionText: t('common.retry'), actionFn: () => load() });
    }
  }
  ...
  function reloadFromFilters() { page = 1; syncUrl(); load(); }
